...
int get_nonvolatile(void* coredump_info, unsigned long no_used, size_t len)
{
    int ret = -1;
    struct file *coredump_conf_file = NULL;

    coredump_conf_file = filp_open(COREDUMP_SETTING_FILE, O_CREAT | O_RDWR, 0664);
    if (!IS_ERR(coredump_conf_file)) {
        ...
        ret = coredump_conf_file->f_op->read(coredump_conf_file, coredump_info, len, &coredump_conf_file->f_pos);
        if(ret == 0) {
            /* "ret =  0" means that file was created newly. write default values. */
            struct coredump_struct *pinfo = (struct coredump_struct*)coredump_info;
            pinfo->limits = DEFAULT_CORE_LIMITSIZE; 
            pinfo->num = DEFAULT_NUM; 
            memset(pinfo->where, 0, MAX_CORE_WHERE);
            strncpy(pinfo->where, DEFAULT_WHERE, strlen(DEFAULT_WHERE));
            ret = coredump_conf_file->f_op->write(coredump_conf_file, coredump_info, len, &coredump_conf_file->f_pos);
        }

        set_fs(fs);

        filp_close( coredump_conf_file, NULL);
    }

    return ret;
}

int set_nonvolatile(void* coredump_info, unsigned long no_used, size_t len)
{
    int ret = -1;
    struct file *coredump_conf_file = NULL;

    coredump_conf_file = filp_open(COREDUMP_SETTING_FILE, O_CREAT | O_WRONLY, 0664);
    if (!IS_ERR(coredump_conf_file)) {
