...
void print_cpu_info(void)
{
   ...
   for_each_online_cpu(cpu) {
      struct task_struct *t = cpu_curr(cpu);
      printk(KERN_INFO "cpu: %d pid:tid %d:%d name:%s state: %ld in:%lld out:%lld\n",
           cpu,t->tgid,t->pid,t->comm,t->state,t->fta_in_timestamp,t->fta_out_timestamp);
      show_stack(t,NULL);
   }
}
EXPORT_SYMBOL(print_cpu_info);
