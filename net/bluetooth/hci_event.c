...
//#undef BT_DBG
//#define BT_DBG(fmt, arg...) printk(KERN_ERR "blueEV %s: " fmt "\n" , __func__ , ## arg)
//Fujitsu WLAN/BT Modification end

int force_acl_master = 1; /* Module param */
static int enable_le = 1;
...
static inline void hci_conn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!ev->status) {
		...
		conn->state = BT_CLOSED;
		if (conn->type == ACL_LINK)
			mgmt_connect_failed(hdev, &ev->bdaddr, conn->type,conn->dst_type,
								ev->status);
	}
...
static inline void hci_disconn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (conn->type == ACL_LINK || conn->type == LE_LINK)
		mgmt_disconnected(hdev, &conn->dst, conn->type, conn->dst_type);
...
not_found:
//Fujitsu WLAN/BT Modification start
	BT_DBG("HCI_OP_LINK_KEY_NEG_REPLY");
//Fujitsu WLAN/BT Modification end
...
static inline void hci_le_conn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (ev->status) {
		mgmt_connect_failed(hdev, &ev->bdaddr, conn->type, conn->dst_type, ev->status);
		hci_proto_connect_cfm(conn, ev->status);
		conn->state = BT_CLOSED;
		hci_conn_del(conn);
		goto unlock;
