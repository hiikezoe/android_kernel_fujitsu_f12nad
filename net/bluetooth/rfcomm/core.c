...
//Fujitsu WLAN/BT Modification start
//#undef BT_DBG
//#define BT_DBG(fmt, arg...) printk(KERN_ERR "blue %s: " fmt "\n" , __func__ , ## arg)
//Fujitsu WLAN/BT Modification end
...
//Fujitsu WLAN/BT Modification start
		BT_DBG(" _sec__  BT_SECURITY_HIGH");
//Fujitsu WLAN/BT Modification end
		auth_type = HCI_AT_GENERAL_BONDING_MITM;
		break;
	case BT_SECURITY_MEDIUM:
//Fujitsu WLAN/BT Modification start
		BT_DBG(" _sec__  BT_SECURITY_MEDIUM");
//Fujitsu WLAN/BT Modification end
		auth_type = HCI_AT_GENERAL_BONDING;
		break;
	default:
//Fujitsu WLAN/BT Modification start
		BT_DBG(" _sec__  BT_SECURITY_else");
//Fujitsu WLAN/BT Modification end
...
static int __rfcomm_dlc_open(struct rfcomm_dlc *d, bdaddr_t *src, bdaddr_t *dst, u8 channel)
{
	...
	int err = 0;
	u8 dlci;
	int ret = 0; // FUJITSU:2012-05-31 343 modification
	int i = 0; // FUJITSU:2012-05-31 343 modification

	BT_DBG("dlc %p state %ld %s %s channel %d",
			d, d->state, batostr(src), batostr(dst), channel);

	BT_DBG("%s called", __func__); // FUJITSU:2012-05-31 343 modification
	...
	if (!s) {
/* FUJITSU:2012-05-31 343 modification start */
		ret = memcmp("00:01:90", batostr(dst), 8);
		BT_DBG("%s memcmp result : %d", __func__, ret);
		if (ret == 0) {
			BT_DBG("%s TANITA should delay enter mdelay", __func__);
			for (i = 0; i < 150; i++) {
				mdelay(1);
			}
			BT_DBG("%s returnd mdelay", __func__);
		}
/* FUJITSU:2012-05-31 343 modification end */
...
static inline void rfcomm_process_dlcs(struct rfcomm_session *s)
{
	...
	BT_DBG("session %p state %ld", s, s->state);
	/*Temp fix by SOLDEL - Sleep in invalid context - Start*/
	if(test_and_clear_bit(RFCOMM_SESSION_PUT, &s->flags)) {
		int cnt = atomic_read(&s->refcnt);
		if (cnt <= 1)
			printk(KERN_ERR"rfcomm_process_dlcs: invalid session cnt: %d\n",cnt);
		rfcomm_session_put(s);
	}
	/*Temp fix by SOLDEL - Sleep in invalid context - Start*/
...
/* FUJITSU:2012-03-23 WLAN/BT rep start */
#if 0
...
#endif
/* FUJITSU:2012-03-23 WLAN/BT rep end */
		if (!s->initiator)
			rfcomm_session_put(s);
...
// Fujitsu WLAN/BT Modification end
	...
	/*Temp fix for sleeping in invalid context by SOLDEL - Start*/
	set_bit(RFCOMM_SESSION_PUT, &s->flags);
	/*Temp fix for sleeping in invalid context by SOLDEL - End*/
