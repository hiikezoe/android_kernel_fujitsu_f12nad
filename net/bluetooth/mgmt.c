									...
									u16 len)
{
	...
	if ((cp->val == test_bit(HCI_PSCAN, &hdev->flags)) &&
	    (0 == test_bit(HCI_ISCAN, &hdev->flags))) {
...
static int read_tx_power_level(struct sock *sk, u16 index, void *data, u16 len)
{
	struct hci_dev *hdev;
...
static int add_remove_le_whitelist(struct sock *sk, u16 index, void *data, u16 len)
{
	struct hci_dev *hdev;
	...
	err = cmd_complete(sk, index,
				MGMT_OP_ADD_REMOVE_FROM_LE_WHITELIST, NULL, 0);
unlock:
	hci_dev_unlock(hdev);
	hci_dev_put(hdev);
...
static int clear_le_whitelist(struct sock *sk, u16 index, void *data, u16 len)
{
	struct hci_dev *hdev;
	int err;
	...
	hdev = hci_dev_get(index);
	if (!hdev)
		return cmd_status(sk, index, MGMT_OP_CLEAR_LE_WHITELIST,
					MGMT_STATUS_INVALID_PARAMS);

	hci_dev_lock(hdev);

	if (!test_bit(HCI_UP, &hdev->flags)) {
		err = cmd_status(sk, index, MGMT_OP_CLEAR_LE_WHITELIST,
				MGMT_STATUS_NOT_POWERED);
		goto unlock;
	}
	...
	if (err < 0) {
		err = cmd_status(sk, index, MGMT_OP_CLEAR_LE_WHITELIST, -err);
		goto unlock;
	}

	err = cmd_complete(sk, index, MGMT_OP_CLEAR_LE_WHITELIST, NULL, 0);
unlock:
	hci_dev_unlock(hdev);
	hci_dev_put(hdev);
...
static int toggle_le_scan(struct sock *sk, u16 index, void *data, u16 len)
{
	struct hci_dev *hdev;
	...
	if (len != sizeof(*cp))
		return cmd_status(sk, index, MGMT_OP_TOGGLE_LE_SCAN,
					MGMT_STATUS_INVALID_PARAMS);
	...
	if (!test_bit(HCI_UP, &hdev->flags)) {
		err = cmd_status(sk, index, MGMT_OP_TOGGLE_LE_SCAN,
				MGMT_STATUS_NOT_POWERED);
		goto unlock;
	}
	...
	err = hci_send_cmd(hdev, HCI_OP_LE_SET_SCAN_PARAM, sizeof(param_cp),
								&param_cp);
	if (err < 0) {
		err = cmd_status(sk, index, MGMT_OP_TOGGLE_LE_SCAN, -err);
		goto unlock;
	}
...
disable_only:
	...
	err = cmd_complete(sk, index, MGMT_OP_TOGGLE_LE_SCAN, NULL, 0);
unlock:
	hci_dev_unlock(hdev);
	hci_dev_put(hdev);
...
static int cancel_resolve_name(struct sock *sk, u16 index, unsigned char *data, u16 len)
{
	...
	struct hci_dev *hdev;
// Fujitsu BlueTi Modify Start
//	struct pending_cmd *cmd;
// Fujitsu BlueTi Modify End
	int err;
								...
								u16 len)
{
	...
	err = hci_send_cmd(hdev, HCI_OP_WRITE_LOCAL_NAME, sizeof(hci_cp),
								&hci_cp);
	/*SOLDEL - Reflect device name in EIR - BEGIN */
	memcpy(hdev->dev_name, mgmt_cp->name, sizeof(hdev->dev_name));
	/*SOLDEL - Reflect device name in EIR - END */
	if (err < 0)
		mgmt_pending_remove(cmd);
...
/* BlueTi End */
	...
	case MGMT_OP_ADD_REMOVE_FROM_LE_WHITELIST:
		err = add_remove_le_whitelist(sk, index, buf + sizeof(*hdr), len);
		break;
	case MGMT_OP_CLEAR_LE_WHITELIST:
		err = clear_le_whitelist(sk, index, buf + sizeof(*hdr), len);
		break;
	case MGMT_OP_TOGGLE_LE_SCAN:
		err = toggle_le_scan(sk, index, buf + sizeof(*hdr), len);
		break;
	default:
...
int mgmt_connected(struct hci_dev *hdev, bdaddr_t *bdaddr, u8 link_type, u8 addr_type)
{
		...
		u8 addr_type)
{
	...
	bacpy(&ev.bdaddr, bdaddr);
	ev.type = link_to_mgmt(type,addr_type);
								...
								u8 addr_type, u8 status)
{
	...
	ev.addr.type = link_to_mgmt(type, addr_type);
	ev.status = status;
				...
				u8 status)
{
	...
	err = cmd_complete(cmd->sk, hdev->id, cmd->opcode, &rp, sizeof(rp));
...
int mgmt_device_found(struct hci_dev *hdev, bdaddr_t *bdaddr, u8 link_type,
				u8 addr_type, u8 *dev_class, s8 rssi, u8 *eir, u8 eir_len)
{
