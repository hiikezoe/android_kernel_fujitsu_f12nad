/* linux/arch/arm/mach-msm/board-wordfish.c
 *
...
extern int wordfish_init_mmc(void);
...
static int wordfish_phy_init_seq[] = {
	0x0C, 0x31,
	0x1D, 0x0D,
	0x1D, 0x10,
	-1
...
static void wordfish_usb_phy_reset(void)
{
	u32 id;
	int ret;
...
static void wordfish_usb_hw_reset(bool enable)
{
	u32 id;
	int ret;
	u32 func;
...
static struct msm_hsusb_platform_data msm_hsusb_pdata = {
	.phy_init_seq		= wordfish_phy_init_seq,
	.phy_reset		= wordfish_usb_phy_reset,
	.hw_reset		= wordfish_usb_hw_reset,
};
...
static struct msm_ts_platform_data wordfish_ts_pdata = {
	.min_x		= 296,
	.max_x		= 3800,
	.min_y		= 296,
	.max_y		= 3800,
...
static struct msm_acpu_clock_platform_data wordfish_clock_data = {
...
static void __init wordfish_init(void)
{
	int rc;

	msm_acpu_clock_init(&wordfish_clock_data);
#if defined(CONFIG_MSM_SERIAL_DEBUGGER)
	msm_serial_debug_init(MSM_UART3_PHYS, INT_UART3,
			      &msm_device_uart3.dev, 1);
#endif
	msm_device_hsusb.dev.platform_data = &msm_hsusb_pdata;
	msm_device_touchscreen.dev.platform_data = &wordfish_ts_pdata;
	platform_add_devices(devices, ARRAY_SIZE(devices));
	msm_hsusb_set_vbus_state(1);
	rc = wordfish_init_mmc();
	if (rc)
		pr_crit("%s: MMC init failure (%d)\n", __func__, rc);
}

static void __init wordfish_fixup(struct machine_desc *desc, struct tag *tags,
...
static void __init wordfish_map_io(void)
{
	msm_map_qsd8x50_io();
	msm_clock_init(msm_clocks_8x50, msm_num_clocks_8x50);
}

MACHINE_START(WORDFISH, "wordfish Board (QCT SURF8250)")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io        = MSM_DEBUG_UART_PHYS,
	.io_pg_offst    = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
	.boot_params	= 0x20000100,
	.fixup		= wordfish_fixup,
	.map_io		= wordfish_map_io,
	.init_irq	= msm_init_irq,
	.init_machine	= wordfish_init,
	.timer		= &msm_timer,
MACHINE_END
