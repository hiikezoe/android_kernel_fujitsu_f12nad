/* linux/arch/arm/mach-msm/board-wordfish-keypad.c
 *
...
#define MODULE_PARAM_PREFIX "board_wordfish."
static int wordfish_ffa;
module_param_named(ffa, wordfish_ffa, int, S_IRUGO | S_IWUSR | S_IWGRP);

#define SCAN_FUNCTION_KEYS 0 /* don't turn this on without updating the ffa support */

static unsigned int wordfish_row_gpios[] = {
	31, 32, 33, 34, 35, 41
...
static unsigned int wordfish_col_gpios[] = { 36, 37, 38, 39, 40 };
...
#define KEYMAP_INDEX(row, col) ((row)*ARRAY_SIZE(wordfish_col_gpios) + (col))

static const unsigned short wordfish_keymap[ARRAY_SIZE(sordfish_col_gpios) * ARRAY_SIZE(wordfish_row_gpios)] = {
...
static const unsigned short wordfish_keymap_ffa[ARRAY_SIZE(wordfish_col_gpios) * ARRAY_SIZE(wordfish_row_gpios)] = {
	/*[KEYMAP_INDEX(0, 0)] = ,*/
	/*[KEYMAP_INDEX(0, 1)] = ,*/
...
static struct gpio_event_matrix_info wordfish_matrix_info = {
	.info.func	= gpio_event_matrix_func,
	.keymap		= wordfish_keymap,
	.output_gpios	= wordfish_row_gpios,
	.input_gpios	= wordfish_col_gpios,
	.noutputs	= ARRAY_SIZE(wordfish_row_gpios),
	.ninputs	= ARRAY_SIZE(wordfish_col_gpios),
...
struct gpio_event_info *wordfish_keypad_info[] = {
	&wordfish_matrix_info.info
};

static struct gpio_event_platform_data wordfish_keypad_data = {
	.name		= "wordfish_keypad",
	.info		= wordfish_keypad_info,
	.info_count	= ARRAY_SIZE(wordfish_keypad_info)
};

static struct platform_device wordfish_keypad_device = {
	.name	= GPIO_EVENT_DEV_NAME,
	.id	= -1,
	.dev	= {
		.platform_data	= &wordfish_keypad_data,
	},
};

static int __init wordfish_init_keypad(void)
{
	if (!machine_is_wordfish())
		return 0;
	if (wordfish_ffa)
		wordfish_matrix_info.keymap = wordfish_keymap_ffa;
	return platform_device_register(&wordfish_keypad_device);
}

device_initcall(wordfish_init_keypad);
