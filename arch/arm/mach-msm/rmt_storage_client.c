...
/* FUJITSU:2012-04-25 shutdown force sync start*/
	unsigned long sync_flg;
/* FUJITSU:2012-04-25 shutdown force sync end*/
...
/* FUJITSU:2012-04-25 shutdown force sync start */
#define SHUTDOWN_SYNC_SEND_FORCE_SYNC   1 << 1
#define SHUTDOWN_SYNC_EXEC_WRITE_EVNT   1 << 2

#define SHUTDOWN_SYNC_WAIT_CNT          30
/* FUJITSU:2012-04-25 shutdown force sync end */
...
/* FUJITSU:2012-04-25 shutdown force sync start*/
		if ( kevent->event.id == RMT_STORAGE_WRITE ){
			rmc->sync_flg |= SHUTDOWN_SYNC_EXEC_WRITE_EVNT;
			if (rmc->sync_flg & SHUTDOWN_SYNC_SEND_FORCE_SYNC ){
                pr_info("%s: force sync flag is on and get Write event ID = 0x%lx\n", __func__, rmc->sync_flg);
				rmc->sync_flg &= ~SHUTDOWN_SYNC_SEND_FORCE_SYNC;
			}
		}
/* FUJITSU:2012-04-25 shutdown force sync end*/
		wake_up(&rmc->event_q);
...
/* FUJITSU:2012-04-25 shutdown force sync start*/
    pr_info("%s: 1st sync working\n", __func__);
	for ( i = 0 ; i < SHUTDOWN_SYNC_WAIT_CNT ; i ++ ){
		if ( rmc->sync_flg == 0x00 )
			break;
		msleep(100);
	}
    if ( i >= SHUTDOWN_SYNC_WAIT_CNT )
        pr_info("%s: sync wait timeout 1\n", __func__);

    pr_info("%s: 2nd send force sync and wait complete\n", __func__);
    rmc->sync_flg = SHUTDOWN_SYNC_SEND_FORCE_SYNC;
    rmt_storage_force_sync(srv->rpc_client);
    for ( i = 0 ; i < SHUTDOWN_SYNC_WAIT_CNT ; i ++ ){
        if ( rmc->sync_flg == 0x00 )
            break;
        pr_info("%s: sleep(%d)\n", __func__, i);
