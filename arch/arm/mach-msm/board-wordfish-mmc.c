/* linux/arch/arm/mach-msm/board-wordfish-mmc.c
 *
...
static int wordfish_sdcc_setup_gpio(int dev_id, unsigned enable)
{
...
static uint32_t sdcc_translate_vdd(struct device *dev, unsigned int vdd)
{
	...
	wordfish_sdcc_setup_gpio(pdev->id, !!vdd);

	/* power down */
	if (vdd == 0) {
#if DEBUG_WORDFISH_MMC
...
static unsigned int wordfish_sdcc_slot_status (struct device *dev)
{
	struct platform_device *pdev;
	uint32_t sdcc_stat;
...
#define WORDFISH_MMC_VDD (MMC_VDD_165_195 | MMC_VDD_20_21 | MMC_VDD_21_22 \
			| MMC_VDD_22_23 | MMC_VDD_23_24 | MMC_VDD_24_25 \
			| MMC_VDD_25_26 | MMC_VDD_26_27 | MMC_VDD_27_28 \
			| MMC_VDD_28_29 | MMC_VDD_29_30)

static struct mmc_platform_data wordfish_sdcc_data = {
	.ocr_mask	= WORDFISH_MMC_VDD/*MMC_VDD_27_28 | MMC_VDD_28_29*/,
	.status		= wordfish_sdcc_slot_status,
	.translate_vdd	= sdcc_translate_vdd,
};

int __init wordfish_init_mmc(void)
{
	vreg_sdcc_enabled = 0;
	...
	msm_add_sdcc(1, &wordfish_sdcc_data, 0, 0);
	msm_add_sdcc(4, &wordfish_sdcc_data, 0, 0);
