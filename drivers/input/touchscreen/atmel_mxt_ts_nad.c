...
#define GET_PANEL_TYPE(tchtype , lcdtype)	(tchtype << 1 | lcdtype)
#define MXT_CONFIG_TCHW_LCDS         GET_PANEL_TYPE(0 , 0)
#define MXT_CONFIG_TCHW_LCDH         GET_PANEL_TYPE(0 , 1)
#define MXT_CONFIG_TCHK_LCDS         GET_PANEL_TYPE(1 , 0)
#define MXT_CONFIG_TCHK_LCDH         GET_PANEL_TYPE(1 , 1)

#define T37_DATA_SIZE                130
#define T40_DATA_SIZE                5
#define T38_DATA_SIZE                8
...
struct mxt_data {
	...
	u16 t7_start_addr;
	u8 t9_ctrl;
	u8 t38_data[T38_DATA_SIZE];
	u8 lcd_type;
	int		ofl_cnt_ofl;
	int		ofl_cnt_touch_event;
	int		ofl_cnt_touch_normal_event;
	int		ofl_cnt_total_event;
	int		ofl_cnt_finger;
	bool	ofl_flag;
	bool	ofl_calibrating;
	bool	ofl_warning;
	bool	ofl_WA_finish;
};

static int		loop_cnt_touching_event		 = 15;		/* Finger Event Count(Pres or Move) */
static int		loop_cnt_touch_normal_event	 = 15;	/* Not warnig Touch Event count */
...
static int mxt_cntr_charging_noise(struct mxt_data *data)
{
	int retval = 0;
	...
	if ((data->mxt_batt_status == POWER_SUPPLY_STATUS_UNKNOWN) ||
		...
		if (data->mxt_chrgon_flag) {
			...
			goto mxt_cntr_charging_noise_exit;
		}
	} else {
		/* Charging */
		if (data->mxt_chrgon_flag) {
...
static int mxt_battery_info(unsigned int batt_status, void *data)
{
	...
	if (pdata->mxt_suspend_flag) {
		mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,
			"[TPD]%s: Cannot Charging Control Noise. Power Supply Status %s\n",
			__func__, mxt_batt_power_str[pdata->mxt_batt_status]);
		return 0;
	}

	retval = mxt_cntr_charging_noise(pdata);
...
static void mxt_input_report(struct mxt_data *data, int single_id)
{
	...
	if ((data->pre_finger_num == 0) && (finger_num == 0)) {
		mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3, "[TPD_DBG]%s: Not Send Event(%d,%d)\n",
			__func__, data->pre_finger_num, finger_num);
		goto mxt_input_report_exit;
	} else {
		mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3, "[TPD_DBG]%s: Send Event(%d,%d)\n",
			__func__, data->pre_finger_num, finger_num);
	}
	finger_num = 0;
	/* Check Finger Num end */
	...
	if (input_sync_flg) {
		input_report_key(input_dev, BTN_TOUCH, finger_num > 0);
		input_sync(input_dev);
		mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3, "[TPD] Touch Event Sync\n");
	}
mxt_input_report_exit:
	data->pre_finger_num = finger_num;
	mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3, "[TPD] pre_finger_num=%d\n", data->pre_finger_num);
				      ...
				      struct mxt_message *message, int id)
{
	...
	/* count up Finger Num(Press or Move) */
	if(data->ofl_cnt_finger >= 0) {
		/* detect Press or Move */
		data->ofl_cnt_finger++;
	}
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	if(data->ofl_cnt_total_event >= 0) {
		data->ofl_cnt_total_event++;
	}
	data->ofl_cnt_finger = 0;
	do {
		if (mxt_read_message(data, &message)) {
			dev_err(dev, "Failed to read message\n");
			goto end;
		...
		if(data->ofl_WA_finish == false) {
			val = mxt_reportid_to_type(data, message.reportid);
			if(MXT_GEN_COMMAND == val) {
				mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:reportid == GEN_COMMAND\n",__func__);
				if(message.message[0]&0x10) {
					data->ofl_calibrating = true;
					mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:now Calibrating\n",__func__);
				} else {
					if(data->ofl_calibrating == true) {
						mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:now Calibration Finish\n",__func__);
						data->ofl_calibrating = false;
						data->ofl_cnt_touch_event = 0;
						data->ofl_cnt_touch_normal_event = 0;
						mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s: ofl_cnt_touch_normal_event = %d\n",__func__,data->ofl_cnt_touch_normal_event);
					}
				}
				if(message.message[0]&0x40) {
					/* Over Flow Flag On */
					mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:OFL On\n",__func__);
					data->ofl_flag = true;
				} else {
					/* Over Flow Flag Off */
					data->ofl_flag = false;
	...
	if(data->ofl_WA_finish == false) {
		if(data->ofl_flag == true) {
			/* Count up short counter */
			data->ofl_cnt_ofl++;
			mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:ofl_cnt_ofl Count UP\n",__func__);
		} else {
			/* Reset short counter */
			data->ofl_cnt_ofl = 0;
		}

		if(data->ofl_cnt_finger == 1) {
			/* Single Touch*/
			data->ofl_cnt_touch_event++;
			mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:touching event Count UP (%d)\n",__func__,data->ofl_cnt_touch_event);
		} else {
			mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:ofl_cnt_touch_event Not single (%d , ofl_cnt_finger = %d)\n",__func__,data->ofl_cnt_touch_event,data->ofl_cnt_finger);
			mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:counter clear\n",__func__);
			data->ofl_cnt_touch_event= 0;
			data->ofl_cnt_touch_normal_event = 0;
		}

		if(data->ofl_cnt_touch_event <= loop_cnt_touching_event) {
			if((data->ofl_cnt_ofl>=3)||(data->ofl_cnt_finger > 1)) {
				if(data->ofl_calibrating == false) {
					mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:OverFlow!! Exec Calibration\n",__func__);
					data->ofl_calibrating = true;
					/* Triger Caribration */
					mxt_write_object(data, MXT_GEN_COMMAND, MXT_COMMAND_CALIBRATE, 1);
				}
				data->ofl_warning = true;
				mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:Overflow detected!! Total Event Count= %d\n",__func__,data->ofl_cnt_total_event);
				/* OverFlow Flags Off */
				data->ofl_flag = false;
				data->ofl_cnt_ofl = 0;
			} else {
				data->ofl_cnt_touch_normal_event++;
				mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s: ofl_cnt_touch_normal_event = %d\n",__func__,data->ofl_cnt_touch_normal_event);
				if (data->ofl_cnt_touch_normal_event > loop_cnt_touch_normal_event) {
					if (data->ofl_WA_finish ==false) {
						mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:Normal event over 15 Total Event Count= %d\n",__func__,data->ofl_cnt_total_event);
						data->ofl_warning = false;
						/* end WA */
						/* T8 Auto Recover Off */
						mxt_multi_write_object(data, MXT_GEN_ACQUIRE,
							(void *)pdata->config_t8_auto_recover_off->config, pdata->config_t8_auto_recover_off->length);

						/* T42 Touch Suppretion Enable */
						pdata->config_t42->config[0] = 0x03;
						mxt_multi_write_object(data, MXT_PROCI_TOUCHSUPPRESSION,
							(void *)pdata->config_t42->config, pdata->config_t42->length);

						data->ofl_WA_finish = true;
					} else {
						mxt_dbg_log(MXT_DEBUG_LOG_LEVEL3,"[TPD_DBG]%s:Allrady Finish WA Total Event Count= %d\n",__func__,data->ofl_cnt_total_event);
					}
				}
			}
		}
...
static int mxt_soft_reset(struct mxt_data *data)
{
	...
	if (reset_timeout >= 100) {
		...
		printk("[TPD]%s: FirmConfig CRC=0x%08X\n", __func__, data->mxt_crc);
	}
...
static ssize_t mxt_oflloop(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	int val1 = 0;
	...
	if ((NULL == buf) || (count < 2) || (5 < count)){
		pr_err("%s: Parameter error\n", __func__);
		return 0;
	}
	
	val1 = mxt_asctobin(buf[0], buf[1]);
	if(count >= 2) {
		val2 = mxt_asctobin(buf[2], buf[3]);
	}
	printk("[TPD_DBG]%s: set OverFlow Loop %d,%d\n",
		__func__, val1, val2);

	loop_cnt_touching_event = val1;
	if(val2 >= 0) {
		loop_cnt_touch_normal_event = val2;
	}
	return count;
...
#else
/* FUJITSU:2012-06-26 Multi Vender Add Start */
static int checkT38UserData(struct mxt_data *data)
{
	static u8 data_w[T38_DATA_SIZE] = 
	{0x54,0x57,0x02,0x04,0x00,0x00,0x00,0x00};
	static u8 data_k[T38_DATA_SIZE] = 
	{0x59,0x4b,0x03,0x04,0x00,0x00,0x00,0x00};
	struct mxt_object *object;
	u16 reg = 0x00;
	...
	memset(data->t38_data, 0,T38_DATA_SIZE);
	object = mxt_get_object(data, MXT_SPT_USERDATA);
	if (!object) {
		retval = -EINVAL;
		return retval;
	}
	...
	reg = object->start_address;
	retval = __mxt_read_reg(data->client, reg,
			T38_DATA_SIZE, data->t38_data);
	if (retval < 0) {
		printk("[TPD]%s: Debug UserData Err retval=%d\n", __func__, retval);
		return retval;
	}

	printk("[TPD]%s:T38_DATA ", __func__);
	for(i = 0 ; i < T38_DATA_SIZE ; i++) {
		printk("[%02x]",data->t38_data[i]);
	}
	printk("\n");

	if (memcmp(data->t38_data, data_w, 8) == 0) {
		printk("[TPD]%s: Touch Vender = W\n", __func__);
		retval = 0;
	} else if(memcmp(data->t38_data, data_k, 8) == 0) {
		printk("[TPD]%s: Touch Vender = K\n", __func__);
		retval = 1;
	} else {
		printk("[TPD]%s: Unknown Touch Vender\n", __func__);
		retval = -1;
	}
	return retval;
}
/* FUJITSU:2012-06-26 Multi Vender Add end */
...
/* FUJITSU:2012-06-26 Multi Vender Add Start */
	tch_type = checkT38UserData(data);
	if(tch_type < 0) {
		printk( KERN_ERR "%s :Unknown Touch Panel\n", __func__ );
		tch_type = 0;		/* Fail Safe */
	}

	data->lcd_type = pdata->get_config_rev();
	if(data->lcd_type < 0) {
		printk( KERN_ERR "%s :Unknown LCD Panel\n", __func__ );
		data->lcd_type = 0;		/* Fail Safe */
	}
	config_rev = GET_PANEL_TYPE(tch_type , data->lcd_type);
	switch (config_rev) {
	case MXT_CONFIG_TCHW_LCDS:
		printk("[TPD_DBG] MXT_CONFIG_TCHW_LCDS\n");
		break;
	case MXT_CONFIG_TCHW_LCDH:
		printk("[TPD_DBG] MXT_CONFIG_TCHW_LCDH\n");
/*		data->pdata = pdata + 1;	*/
		break;
	case MXT_CONFIG_TCHK_LCDS:
		printk("[TPD_DBG] MXT_CONFIG_TCHK_LCDS\n");
/*		data->pdata = pdata + 2;	*/
		break;
	case MXT_CONFIG_TCHK_LCDH:
		printk("[TPD_DBG] MXT_CONFIG_TCHK_LCDH\n");
/*		data->pdata = pdata + 3;	*/
		break;
	default:
		printk("[TPD_DBG] Unknown Panel\n");
		break;
	}
/* FUJITSU:2012-06-26 Multi Vender Add End */
...
static DEVICE_ATTR(oflloop, 0664, NULL, mxt_oflloop);
...
static int mxt_resume(struct device *dev)
{
	...
	/* T42 Touch Suppretion Disable */
	pdata->config_t42->config[0] = 0x00;
	mxt_multi_write_object(data, MXT_PROCI_TOUCHSUPPRESSION,
		(void *)pdata->config_t42->config, pdata->config_t42->length);
	...
	mutex_unlock(&input_dev->mutex);
	printk("[TPD_DBG]%s:Start WA\n",__func__);
	/* Counter clear & Count start */
	data->ofl_cnt_ofl = 0;
	data->ofl_cnt_touch_event= 0;
	data->ofl_cnt_touch_normal_event = 0;
	printk("[TPD_DBG]%s: ofl_cnt_touch_normal_event = %d\n",__func__,data->ofl_cnt_touch_normal_event);
	data->ofl_cnt_total_event = 0;
	data->ofl_cnt_finger = 0;
	/* ofl Check Flag reset */
	data->ofl_flag = false;
	data->ofl_calibrating = true;	/* Now Caribrating */
	data->ofl_warning = false;
	data->ofl_WA_finish = false;
	return 0;
}
#endif
/* FUJITSU:2012-04-03 MXT_ENABLE_ORIGINAL_FUNCTION end */
...
/* FUJITSU:2012-04-16 Multi Vender Add Start */
		verdata[7]  = data->t38_data[0];
		verdata[8]  = data->t38_data[1];
		verdata[9]  = data->t38_data[2];
		verdata[10] = data->t38_data[3];

		verdata[11] = data->lcd_type;
/* FUJITSU:2012-04-16 Multi Vender Add End */
		...
		const struct i2c_device_id *id)
{
	...
	int retval = 0;
	int devno;
/*	int config_rev = 0;	*/		/* FUJITSU:2012-06-26 Multi Vender Del */
...
/* FUJITSU:2012-06-26 Multi Vender Del Start */
#if 0
	config_rev = pdata->get_config_rev();
	switch (config_rev) {
	case MXT_CONFIG_REV_0:
		printk("[TPD_DBG] MXT_CONFIG_REV_0\n");
		break;
	case MXT_CONFIG_REV_1:
		printk("[TPD_DBG] MXT_CONFIG_REV_1\n");
		data->pdata = pdata + 1;
	}
#endif
/* FUJITSU:2012-06-26 Multi Vender Del End */
	if (pdata->init_hw)
		error = pdata->init_hw(true);
	...
	memset(data->finger, 0x00, sizeof(struct mxt_finger) * MXT_MAX_FINGER);
	data->pre_finger_num = 0;
	...
	printk("[TPD]%s: probe complete (Built %s @ %s)\n",
