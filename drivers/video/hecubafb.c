/*
 * linux/drivers/video/hecubafb.c -- FB driver for Hecuba/pollo controller
 *
 ...
 * This work was possible because of pollo display code from E-Ink's website
 ...
 * The actual display chip is called pollo and the interface electronics
...
static void pollo_send_data(struct hecubafb_par *par, unsigned char data)
{
	/* set data */
...
static void pollo_send_command(struct hecubafb_par *par, unsigned char data)
{
	/* command so set CD to high */
	par->board->set_ctl(par, HCB_CD_BIT, 1);

	/* actually strobe with command */
	pollo_send_data(par, data);
...
static void hecubafb_dpy_update(struct hecubafb_par *par)
{
	...
	pollo_send_command(par, POLLO_START_NEW_IMG);

	for (i=0; i < (DPY_W*DPY_H/8); i++) {
		pollo_send_data(par, *(buf++));
	}

	pollo_send_command(par, POLLO_STOP_IMG_DATA);
	pollo_send_command(par, POLLO_DISPLAY_IMG);
}
...
MODULE_DESCRIPTION("fbdev driver for Hecuba/pollo controller");
