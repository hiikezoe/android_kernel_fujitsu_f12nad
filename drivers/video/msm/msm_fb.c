...
#include "mdp.h"
#include "mdp4.h"
/* FUJITSU:2012-02-13 GammaCorrection start */  
extern void fj_mdp_set_lut_table_index(int i);
DEFINE_MUTEX(msm_fb_ioctl_lut_sem);
#define GAMMA_LOG printk
extern unsigned char *gammaR_TablePtr[];
extern unsigned char *gammaG_TablePtr[];
extern unsigned char *gammaB_TablePtr[];
/* FUJITSU:2012-02-13 GammaCorrection end */
...
/* FUJITSU:2012-03-07 GammaCorrection start */ 
int gamma_condition[]={0,0,0};
...
//Check command and priority
	char input;
	char priority[4] = {' ', ' ', ' ',' '};
	char cmd[4] = {' ', ' ', ' ',' '};
	unsigned int status;	
	int j;
//Understand cmd(on/off)
	char tmp[4] = {' ', ' ', ' ',' '};
	j = 0;
	status = 0;
	...
	if (cmd[0] == 'l' && cmd[1] == 'u' && cmd[2] == 't') {

		if (priority[0] == '1' || priority[0] == '2' || 
			priority[0] == '3' || priority[0] == '4') {
			table = 0;
		} else if (priority[0] == '5' || priority[0] == '6') {
			table=1;
		} else {
			return count;
		}
		...
		if (cmd[3] == 'r') {
			ptr = &gammaR_TablePtr[table][0];
		} else if (cmd[3] == 'g') {
			ptr = &gammaG_TablePtr[table][0];
		} else if (cmd[3] == 'b') {
			ptr = &gammaB_TablePtr[table][0];
		} else {
			return count;
	    }
	...
	//Decide table for gamma correction by cmd(on/off) with priority.
	if (cmd[0] == 'o' && cmd[1] == 'n') {
		switch (priority[0]) {
		case '0':
			gamma_condition[0] = 0;
		...
		case '8':
			...
			break;
		default:
			GAMMA_LOG("msm_fb.c: Invalid command is inputted!!\n");
			return count;
		}
	...
	if (gamma_condition[2] == 1) {
		table = 2;
		GAMMA_LOG("Set gammatable for MHL. table=%d\n",table);
	} else if (gamma_condition[1] == 1) {
		table = 1;
		GAMMA_LOG("Set gammatable for privacy view. table=%d\n",table);
	} else if (gamma_condition[0] == 1) {
		table = 0;
		GAMMA_LOG("Set gammatable for libMmp. table=%d\n",table);
	} else {
		table = -1;
		GAMMA_LOG("Set gammatable for default. table=%d\n",table);
	}

	//Here is for in case of only priority was inputted.
	if (cmd[0] == '0') {
		table = -1;
		GAMMA_LOG("Set gammatable for default. table=%d\n",table);
	} else if (cmd[0] == '1' || cmd[0] == '2' || cmd[0] == '3' 
													|| cmd[0] == '4') {
		table = 0;
		GAMMA_LOG("Set gammatable for libMmp. table=%d\n",table);
	} else if (cmd[0] == '5' || cmd[0] == '6') {
		table = 1;
		GAMMA_LOG("Set gammatable for privacy view. table=%d\n",table);
	} else if (cmd[0] == '8') {
		table = 2;
	...
	fj_mdp_set_lut_table_index(table);
...
/* FUJITSU:2012-03-07 GammaCorrection end */ 
...
/* FUJITSU:2012-02-06 GammaCorrection start */ 
	rc = device_create_file(mfd->fbi->dev, &dev_attr_gamma);
/* FUJITSU:2012-02-06 GammaCorrection end */  
	if (rc)
...
/* FUJITSU:2012-02-06 GammaCorrection start */ 
	device_remove_file(mfd->fbi->dev, &dev_attr_gamma);
/* FUJITSU:2012-02-06 GammaCorrection end */ 
}
...
#if 1 /* FUJITSU:2012-02-27 DISP add LCD actual size start */
	var->height = panel_info->actual_height,	/* height of picture in mm */
	var->width = panel_info->actual_width,	/* width of picture in mm */
#else
/* FUJITSU:2011-12-01 modify start */
//	var->height = -1,	/* height of picture in mm */
//	var->width = -1,	/* width of picture in mm */
#if defined(CONFIG_MACH_FJI12) || defined(CONFIG_MACH_F11APO) || defined(CONFIG_MACH_F12APON)	
...
#endif
/* FUJITSU:2011-12-01 modify end */
#endif /* FUJITSU:2012-02-27 DISP add LCD actual size end */
	...
	if (mfd->dest == DISPLAY_LCD) {
/* FUJITSU:2012-04-17 DISP Support for decimal point start */
//		var->reserved[4] = panel_info->lcd.refx100 / 100;
		var->reserved[4] = panel_info->lcd.refx100;
/* FUJITSU:2012-04-17 DISP Support for decimal point end */
	} else {
		if (panel_info->type == MIPI_VIDEO_PANEL) {
/* FUJITSU:2012-04-17 DISP Support for decimal point start */
//			var->reserved[4] = panel_info->mipi.frame_rate;
			var->reserved[4] = panel_info->mipi.frame_rate * 100;
/* FUJITSU:2012-04-17 DISP Support for decimal point end */
		} else {
/* FUJITSU:2012-04-17 DISP Support for decimal point start */
//			var->reserved[4] = panel_info->clk_rate /
			var->reserved[4] = (panel_info->clk_rate * 100) /
/* FUJITSU:2012-04-17 DISP Support for decimal point end */
