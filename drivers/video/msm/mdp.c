...
#define GAMMA_DEFAULT_LOG printk
...
static u16 fj_mdp_stored_cmap_r[256] =
{
...
static u16 fj_mdp_stored_cmap_g[256] =
{
0x0000,0x0001,0x0002,0x0003,0x0004,0x0005,0x0006,0x0007,0x0008,0x0009,0x000A,0x000B,0x000C,0x000D,0x000E,0x000F,
0x0010,0x0011,0x0012,0x0013,0x0014,0x0015,0x0016,0x0017,0x0018,0x0019,0x001A,0x001B,0x001C,0x001D,0x001E,0x001F,
0x0020,0x0021,0x0022,0x0023,0x0024,0x0025,0x0026,0x0027,0x0028,0x0029,0x002A,0x002B,0x002C,0x002D,0x002E,0x002F,
...
static u16 fj_mdp_stored_cmap_b[256] =
{
0x0000,0x0001,0x0002,0x0003,0x0004,0x0005,0x0006,0x0007,0x0008,0x0009,0x000A,0x000B,0x000C,0x000D,0x000E,0x000F,
0x0010,0x0011,0x0012,0x0013,0x0014,0x0015,0x0016,0x0017,0x0018,0x0019,0x001A,0x001B,0x001C,0x001D,0x001E,0x001F,
0x0020,0x0021,0x0022,0x0023,0x0024,0x0025,0x0026,0x0027,0x0028,0x0029,0x002A,0x002B,0x002C,0x002D,0x002E,0x002F,
...
static int fj_mdp_lut_tablenum = -1;
static int fj_mdp_lut_tablenum_updated = 1;

static int fj_mdp_lut_set_colors(int table_no)
{
	int i;
	u16 r, g, b;
	...
	if (table_no >= NO_OF_GAMMA_TABLES) {
		return -1;
	}
	if (fj_mdp_lut_tablenum_updated) {
		fj_mdp_lut_tablenum_updated = 0;
	} else {
		return 0;
	}
	...
	for (i = 0; i < 256; i++)
	{	
		r = gammaR_TablePtr[table_no][i];
		g = gammaG_TablePtr[table_no][i];
		b = gammaB_TablePtr[table_no][i];	
...
/* FUJITSU:2012-02-06 GammaCorrection start */
		if (copy_from_user(&r, cmap->red+i, sizeof(r)) ||
		    copy_from_user(&g, cmap->green+i, sizeof(g)) ||
		    copy_from_user(&b, cmap->blue+i, sizeof(b)))
		    {
			//return -EFAULT;		
			r = *(cmap->red+i);
			g = *(cmap->green+i);
...
/* FUJITSU:2012-02-06 GammaCorrection start */
		fj_mdp_stored_cmap_r[cmap->start+i] = r;
		fj_mdp_stored_cmap_g[cmap->start+i] = g;
		fj_mdp_stored_cmap_b[cmap->start+i] = b;
	}
		if (fj_mdp_lut_tablenum >= 0) {
			fj_mdp_lut_set_colors(fj_mdp_lut_tablenum);
		}
/* FUJITSU:2012-02-06 GammaCorrection end */	
	return 0;
}
...
/* FUJITSU:2012-02-06 GammaCorrection start */ 
void fj_mdp_set_lut_table_index(int i)
	{
	struct fb_cmap cmap;
	
	fj_mdp_lut_tablenum = i;
	fj_mdp_lut_tablenum_updated = 1;
	...
	cmap.start=0;
	cmap.green = &fj_mdp_stored_cmap_g[0];
	cmap.blue = &fj_mdp_stored_cmap_b[0];
	cmap.red = &fj_mdp_stored_cmap_r[0];
	 
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	mdp_lut_hw_update(&cmap);
/* FUJITSU:2012-03-26 DISP support LCDC panel start */
	if (mdp_prim_panel_type == LCDC_PANEL) {
		MDP_OUTP(MDP_BASE + 0x90070, (mdp_lut_i << 10) | 0x17);
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
	} else {
/* FUJITSU:2012-03-26 DISP support LCDC panel end */
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
		mutex_lock(&mdp_lut_push_sem);
		mdp_lut_push = 1;
		mdp_lut_push_i = mdp_lut_i;
