...
#define	USB_SERIAL	1		/* FUJITSU:2012-01-23 DN TestMode_001 */
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
#if 0
...
/* FUJITSU:2012-01-23 DN TestMode_001 end */
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
	if ((memcmp(driver->client_map[i].name,"TestModeService",15)==0) && (testmodepid == 0)){
		testmodepid = current->tgid;
	}
/* FUJITSU:2012-01-23 DN TestMode_001 end */
}
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
#if USB_SERIAL 
	else if(iocmd == DIAG_IOCTL_TESTCOM_USB)
	{
#ifdef DIAG_DEBUG
	printk(KERN_INFO "DIAG_IOCTL_TESTCOM_USB\n");
#endif
		for (i = 0; i < driver->num_clients; i++)
		{
			if (driver->client_map[i].pid == current->tgid)
		...
		driver->data_ready[i] = TESTCOM_TYPE;
		driver->connect_type = DIAG_CONNECT_TYPE_USB;
		success = 1;
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
#if USB_SERIAL 
  if(driver->connect_type != DIAG_CONNECT_TYPE_USB)
  {
    int r;
	wait_event_interruptible(driver->smdwait_q, driver->smd_data_ready);
	mutex_lock(&driver->diagchar_mutex);
#ifdef DIAG_DEBUG
	printk(KERN_INFO "wakeup read interruptible FTM index = %d data_ready = %x\n", index,driver->smd_data_ready);
#endif
    if (copy_to_user(buf, (void *)driver->write_ptr_1->buf,
         driver->write_ptr_1->length)) {
      ret = -EFAULT;
      goto exit;
    }
    ret = driver->write_ptr_1->length;
    driver->smd_data_ready = 0;
 	driver->in_busy_1 = 0;
	r = smd_read_avail(driver->ch);
#ifdef DIAG_DEBUG
	printk(KERN_INFO "diagchar_read ftm r = %d\n", r);
#endif
    goto exit;
  }
#endif
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
#if USB_SERIAL 
  if (driver->data_ready[index] & TESTCOM_TYPE) {
    /*Copy the type of data being passed*/
    data_type = driver->data_ready[index] & TESTCOM_TYPE;
    ...
    driver->data_ready[index] ^= TESTCOM_TYPE;
    goto exit;
  }
#endif
/* FUJITSU:2012-01-23 DN TestMode_001 end */
...
/* FUJITSU:2012-01-23 DN TestMode_001 start	*/
	...
	if(driver->connect_type != DIAG_CONNECT_TYPE_USB)
    {
        ...
        if (!buf_copy) {
			printk(KERN_INFO "diagchar_write 0 ftm : diagmem_alloc failed\n");
          ret = -ENOMEM;
          goto fail_free_buf_copyc;
        }
        err = copy_from_user(buf_copy, buf, payload_size);
        if (err) {
          printk(KERN_INFO "diagchar_write 0 ftm : copy_from_user failed\n");
          ret = -EFAULT;
          goto fail_free_buf_copyc;
        }
#ifdef DIAG_DEBUG
		printk(KERN_INFO "->diagchar_write to FTM size = %d\n" ,payload_size);
#endif
		hdlc.dest_ptr = driver->hdlc_buf;
		hdlc.dest_size = USB_MAX_OUT_BUF;
		hdlc.src_ptr = buf_copy;
		hdlc.src_size = payload_size;
		hdlc.src_idx = 0;
		hdlc.dest_idx = 0;
		hdlc.escaping = 0;
		...
		if ((driver->ch) && (ret) && (hdlc.dest_idx > 3))
		{
#ifdef DIAG_DEBUG
			printk(KERN_INFO "!!!TRUE writing data to SMD, pkt length %d \n", hdlc.dest_idx - 3);
			print_hex_dump(KERN_DEBUG, "Written Packet Data to SMD: ", 16,
			       1, DUMP_PREFIX_ADDRESS, driver->hdlc_buf, hdlc.dest_idx - 3, 1);
#endif
        	smd_write(driver->ch, driver->hdlc_buf, hdlc.dest_idx - 3);
#ifdef DIAG_DEBUG
			printk(KERN_INFO "FTM writing data to SMD, pkt length %d \n", payload_size);
			print_hex_dump(KERN_DEBUG, "Written Packet Data to SMD: ", 16,
			       	1, DUMP_PREFIX_ADDRESS, buf_copy, payload_size, 1);
#endif
		}
        mutex_unlock(&driver->diagchar_mutex);
...
/* FUJITSU:2012-01-23 DN TestMode_001 start	*/
#if USB_SERIAL
  /* Get the packet type testcommand data */
  err = copy_from_user(&pkt_type, buf, 1);

  /* check TEST COMMAND data ID */
  if(((unsigned char )pkt_type == 0x7E ) || ((count == 1) && ((unsigned char )pkt_type == 0x2F))
  || ((count == 1) && ((unsigned char )pkt_type == 0xA3)) || ((count == 1) && ((unsigned char )pkt_type == 0xAA)))
  {
    payload_size = count;
    buf_copy = diagmem_alloc(driver, payload_size, POOL_TYPE_COPY);
    if (!buf_copy) {
    ...
    err = copy_from_user(buf_copy, buf, payload_size);
    if (err) {
      printk(KERN_INFO "diagchar_write 1  : copy_from_user failed\n");
      ret = -EFAULT;
      goto fail_free_copy;
    }
    mutex_lock(&driver->diagchar_mutex);
    ret = -EFAULT;
    driver->write_ptr_1->buf = buf_copy;
    driver->write_ptr_1->length = payload_size;

    if(driver->testcom_write_buf != NULL)
    {
      timeout = 1000;
      /* wait diagfwd_write_complete */
      while(driver->testcom_write_buf != NULL)
      {
        msleep(1);
        timeout--;
        if(timeout == 0)
        {
          printk(KERN_INFO "diagchar : waiting diagfwd_write_complete timeout\n");
          break;
        }
      }
    }
    /* save write buffer address */
    driver->testcom_write_buf = driver->write_ptr_1->buf;
    ret = 0;
    err = usb_diag_write(driver->legacy_ch, driver->write_ptr_1);
    if (err) {
      ret = -EIO;
    }
    driver->used = 0;
...
/* FUJITSU:2012-01-23 DN TestMode_001 start	*/
#if USB_SERIAL
		driver->testcom_write_buf = NULL;
		driver->connect_type = DIAG_CONNECT_TYPE_USB;
		init_waitqueue_head(&driver->smdwait_q);
		driver->smd_data_ready = 0;
#endif
/* FUJITSU:2012-01-23 DN TestMode_001 end	*/
