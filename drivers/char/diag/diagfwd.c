...
/* FUJITSU:2011-10-12 DEBUG_DIAG start	*/
#define USB_SERIAL	1
/* FUJITSU:2011-10-12 DEBUG_DIAG end		*/
...
/* FUJITSU:2012-01-23 DN TestMode_001 */
#if 0
...
extern unsigned int testmodepid;
...
#if USB_SERIAL
				driver->in_busy_1 = 1;
				driver->write_ptr_1->buf = buf;
				driver->write_ptr_1->length = r;
				if(driver->connect_type != DIAG_CONNECT_TYPE_USB)
				{
					mutex_lock(&driver->diagchar_mutex);
					driver->smd_data_ready = TESTFTM_TYPE;
					wake_up_interruptible(&driver->smdwait_q);
					mutex_unlock(&driver->diagchar_mutex);
#ifdef DIAG_DEBUG
					printk(KERN_INFO "diag smd kick wake_up \n");
#endif
				}
				else
				{
			...
			for (i = 0; i < driver->poolsize_write_struct; i++)
				if (driver->buf_tbl[i].length == 0) {
					...
					driver->buf_tbl[i].length =
								 driver->used;
#if USB_SERIAL		/* FUJITSU:2012-01-23 DN TestMode_001	*/
#ifdef DIAG_DEBUG
...
#endif
#endif /* USB_SERIAL */		/* FUJITSU:2012-01-23 DN TestMode_001	*/
					break;
				}
		}
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
void diag_update_sleeping_process_for_testcom(void)
{
	int i;

#ifdef DIAG_DEBUG
	printk(KERN_INFO "diag_update_sleeping_process_for_testcom \n");
#endif	
	mutex_lock(&driver->diagchar_mutex);
	if (!factory_mode()){
		for (i = 0; i < driver->num_clients; i++)
			if (((int *)(driver->client_map))[i] != 0) {
				driver->data_ready[i] |= TESTCOM_TYPE;
			}
	}else{
		for (i = 0; i < driver->num_clients; i++)
		if (driver->client_map[i].pid != 0) {
			if (driver->client_map[i].pid ==  testmodepid) {
				driver->data_ready[i] |= TESTCOM_TYPE;
				break;
			}
		}
...
/* FUJITSU:2012-01-23 DN TestMode_001 start */
		...
		driver->pkt_length = driver->read_len_legacy;
		diag_update_pkt_buffer(driver->usb_buf_out);
		diag_update_sleeping_process_for_testcom();
		return 0;
	}
#endif
/* FUJITSU:2012-01-23 DN TestMode_001 end */
...
/* FUJITSU:2012-01-23 DN TestMode_001 start	*/
	...
	else if(driver->testcom_write_buf == buf) {
