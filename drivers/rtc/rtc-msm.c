...
extern struct mutex time_sync_mutex;
/* ADD-E 2012-06-26 Time Sync Mutex */

/* FUJITSU:2011-09-13 Anti-rollback Clock start */
extern long g_offset;
/* FUJITSU:2011-09-13 Anti-rollback Clock end */
...
/* FUJITSU:2011-09-13 Anti-rollback Clock start */
		tm = &tmptm;
		memcpy(tm, rtc_args->tm, sizeof(struct rtc_time));
		tm->tm_year -= 1900;
		tm->tm_mon--;
		rtc_tm_to_time(tm, &tmpsec); 
		tmpsec += g_offset;
		rtc_time_to_tm(tmpsec, tm);
		tm->tm_year += 1900;
		tm->tm_mon++;
...
//		diff = rtc_pdata->rtcalarm_time - now;
		diff = rtc_pdata->rtcalarm_time - now - g_offset;
/* FUJITSU:2011-09-13 Anti-rollback Clock end */
		if (diff <= 0) {
