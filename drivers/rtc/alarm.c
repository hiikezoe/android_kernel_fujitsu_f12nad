...
/* ADD-S 2012-06-26 Time Sync Mutex */
DEFINE_MUTEX(time_sync_mutex);
/* ADD-E 2012-06-26 Time Sync Mutex */
...
/* FUJITSU:2012-02-11 Anti-rollback Clock start */
int alarm_set_time(struct timespec new_time)
{
        int i;
        int ret;
        unsigned long flags;
...
int alarm_set_rtc(struct timespec new_time)
{
	...
	mutex_lock(&alarm_setrtc_mutex);
	// 2012/04/19 Auto time adjust : start
	//spin_lock_irqsave(&alarm_slock, flags);
	// 2012/04/19 Auto time adjust : end
	wake_lock(&alarm_rtc_wake_lock);
	// 2012/04/19 Auto time adjust : start
	//getnstimeofday(&tmp_time);
	//for (i = 0; i < ANDROID_ALARM_SYSTEMTIME; i++) {
...
//	ret = rtc_set_time(alarm_rtc_dev, &rtc_new_rtc_time);
	ret = rtc_set_rtc(alarm_rtc_dev, &rtc_new_rtc_time);
...
/* FUJITSU:2012-02-11 Anti-rollback Clock start */
	// initialize offset
	g_offset = 0;
/* FUJITSU:2012-08-11 Anti-rollback Clock end */
