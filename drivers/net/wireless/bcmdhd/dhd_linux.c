...
#include <linux/delay.h>	// Fujitsu 2012.1.8 WLAN/BT add start  ** zantei **
#endif
...
// Fujitsu 2012.5.11 WLAN/BT add start
extern bool sdio_pol_off_tts; // WLAN/BT
// Fujitsu 2012.5.11 WLAN/BT add end
...
{
#if 1	 // Fujitsu 2012.04.26 WLAN/BT 12_1ST_GENERIC 0376 del start
...
#if 0	 // Fujitsu 2012.05.18 WLAN/BT del start
				dhd_wl_ioctl_cmd(dhd, WLC_SET_PM, (char *)&power_mode,
				                 sizeof(power_mode), TRUE, 0);
#endif	 // Fujitsu 2012.05.18 WLAN/BT del end
...
#endif	 // Fujitsu 2012.04.26 WLAN/BT 12_1ST_GENERIC 0376 del end
	return 0;
}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	uint32 maxassoc = 8; // Fujitsu 2012.01.23 WLAN/BT add
// Fujitsu 2012.03.10 WLAN/BT add start 
	uint32 nmode = 0;
    bool  bAPmode = FALSE; 

#if 1
	if (strstr(fw_path, "_apsta") != NULL) {
        	DHD_TRACE(("%s bAPmode = TRUE \n", __FUNCTION__)); // Fujitsu 2012.03.01 WLAN/BT add
            bAPmode = TRUE;
        }
#endif
// Fujitsu 2012.03.10 WLAN/BT add end 
	DHD_TRACE(("Enter %s\n", __FUNCTION__));
	dhd->op_mode = 0;
#ifdef GET_CUSTOM_MAC_ENABLE
...
// Fujitsu 2012.03.10 WLAN/BT add start
         ...
         if (bAPmode== TRUE) {
                ...
                if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
                        DHD_ERROR(("%s: Setting NMODE %d\n", __FUNCTION__, ret));

                bcm_mkiovar("maxassoc", (char*)&maxassoc, 4, iovbuf, sizeof(iovbuf));
                if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) 
                    DHD_ERROR(("%s: FAIL to set MAX_ASSOC:%d ---- ret=%d\n", __FUNCTION__, maxassoc, ret));
        }
#endif
        ...
        if (bAPmode== FALSE) {
                ...
                DHD_TRACE(("%s check bAPmode: FALSE \n", __FUNCTION__)); // Fujitsu 2012.03.01 WLAN/BT add
                bcm_mkiovar("sgi_rx", (char*) &temp_rx, 4, iovbuf, sizeof(iovbuf));
                if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
                        DHD_ERROR(("%s: Setting SGI_RX %d\n", __FUNCTION__, ret));

                bcm_mkiovar("sgi_tx", (char*) &temp_tx, 4, iovbuf, sizeof(iovbuf));
                if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
                        DHD_ERROR(("%s: Setting SGI_TX %d\n", __FUNCTION__, ret));
        }
#endif

#if 0
// Fujitsu 2012.02.23 WLAN/BT add start

// Fujitsu 2012.01.23 WLAN/BT add start
	bcm_mkiovar("maxassoc", (char*)&maxassoc, 4, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0) {
		DHD_ERROR(("%s: FAIL to set MAX_ASSOC:%d ---- ret=%d\n", __FUNCTION__, maxassoc, ret));
	}else{
		DHD_ERROR(("%s: SUCCESS to set MAX_ASSOC:%d ---- ret=%d\n", __FUNCTION__, maxassoc, ret));
	}
// Fujitsu 2012.02.23 WLAN/BT add end
#endif
// Fujitsu 2012.03.10 WLAN/BT add end
...
// Fujitsu 2012.04.25 WLAN/BT add start
#if 1
  if (strstr(fw_path, "fw_bcmdhd.bin") != NULL) {
      uint32 cac = 1;
      DHD_ERROR(("%s: STA - enabling admission control\n", __FUNCTION__));
      ...
      DHD_ERROR(("%s: not STA\n", __FUNCTION__));
#endif
// Fujitsu 2012.04.25 WLAN/BT add end
	/* Force STA UP */
...
{
	int error = 0;
// Fujitsu 2012.06.21 WLAN/BT change start
//	int timeout = 5000;		// Fujitsu 2012.5.11 WLAN/BT add
	int timeout = 15000;		// Fujitsu 2012.5.11 WLAN/BT add
// Fujitsu 2012.06.21 WLAN/BT change end
...
#if 0
// Fujitsu 2012.03.27 WLAN/BT change start
#define  DHD_INIT_WAIT_TIME  2
//	DHD_ERROR(("wait 11s (temporary: WLAN).\n"));
	DHD_ERROR(("wait %ds (for SDIO polling OFF: WLAN).\n", DHD_INIT_WAIT_TIME));
//	msleep(1000 * 11);	// Fujitsu 2012.1.8 WLAN/BT add start  ** zantei ** wait 11s
	msleep(1000 * DHD_INIT_WAIT_TIME);
// Fujitsu 2012.03.27 WLAN/BT change end
#else
	...
	// Max 15Sec wait. polling for [sdio_pol_off_tts]. // Fujitsu 2012.06.21 WLAN/BT change
	DHD_ERROR(("%s: wait start\n", __FUNCTION__));
	while( !sdio_pol_off_tts && timeout ){
		timeout = schedule_timeout( timeout );
	}
// Fujitsu 2012.06.21 WLAN/BT change start
	if (timeout) {
		DHD_ERROR(("%s: wait end (time left %dms)\n", __FUNCTION__, jiffies_to_msecs(timeout)));
	} else {
		DHD_ERROR(("%s: wait timeout\n", __FUNCTION__));
		printk(KERN_CRIT "bt_wlan: %s(), DHD initialization failure\n", __func__);
	}
// Fujitsu 2012.06.21 WLAN/BT change end
