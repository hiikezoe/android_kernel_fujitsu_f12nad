...
// Fujitsu 2012.2.21 WLAN/BT chg start
//u32 wl_dbg_level = WL_DBG_ERR;
u32 wl_dbg_level = 0xFF;
// Fujitsu 2012.2.21 WLAN/BT chg end

// Fujitsu 2012.02.07 WLAN/BT add start
#define DAEMONIZE(a) daemonize(a); \
	allow_signal(SIGKILL); \
	allow_signal(SIGTERM);
// Fujitsu 2012.02.07 WLAN/BT add end
...
wl_set_set_cipher(struct net_device *dev, struct cfg80211_connect_params *sme)
{
	...
	if (is_wps_conn(sme)) {
// Fujitsu 2012.04.23 WLAN/BT qcomQA 174 change start
#if 1 // for 5.1.5 patch
		WL_ERR(("#### WPS privacy=%d\n", sme->privacy));
		if (sme->privacy)
			err = wldev_iovar_setint_bsscfg(dev, "wsec", 4, bssidx);
		else
			err = wldev_iovar_setint_bsscfg(dev, "wsec", 0, bssidx);
#else  // for 5.1.5 patch
		err = wldev_iovar_setint_bsscfg(dev, "wsec", 4, bssidx);
#endif // for 5.1.5 patch
// Fujitsu 2012.04.23 WLAN/BT qcomQA 174 change end
	} else {
...
wl_validate_wpa2ie(struct net_device *dev, bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	u32 wpa_auth = 0;
	u8* tmp;
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 add start
#if 1	// 20120426 mixmode
	u16 count;
#endif	// 20120426 mixmode
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 add end
...
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 add start
#if 1	// 20120426 mixmode
	count = ucast->count.low | (ucast->count.high << 8);
#endif	// 20120426 mixmode
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 add end
	...
	/* check the AKM */
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 rep start
#if 1	// 20120426 mixmode
	mgmt = (wpa_suite_auth_key_mgmt_t *)&ucast->list[count];
#else	// 20120426 mixmode
	mgmt = (wpa_suite_auth_key_mgmt_t *)&ucast->list[1];
#endif	// 20120426 mixmode
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 rep end
...
// Fujitsu 2012.05.04 WLAN/BT qcomQA 160-7 add start
#if 1	// 20120426 mixmode
//wl_validate_wpamix_ie(struct net_device *dev, bcm_tlv_t *wpa2ie, wpa_ie_fixed_t *wpaie, s32 bssidx)
s32 wl_validate_wpamix_ie(struct net_device *dev, bcm_tlv_t *wpa2ie, wpa_ie_fixed_t *wpaie, s32 bssidx)
{
	s32 err;
	s32 auth[2];
	s32 wsec[2];
	...
	if (!wpa2ie || !wpaie) {
		WL_ERR(("wpa2ie or wpaie NULL\n"));
		return BCME_ERROR;
	}

	/* Parse WPA2 IE */
	if ((err = wl_validate_wpa2ie(dev, wpa2ie, bssidx)) < 0) {
		WL_ERR(("WPA2 IE error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_getint_bsscfg(dev, "auth", &auth[0], bssidx)) < 0) {
		WL_ERR(("WPA2 IE  auth error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_getint_bsscfg(dev, "wsec", &wsec[0], bssidx)) < 0) {
		WL_ERR(("WPA2 IE wsec error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_getint_bsscfg(dev, "wpa_auth", &wpa_auth[0], bssidx)) < 0) {
		WL_ERR(("WPA2 IE wpa_auth error %d\n", err));
		return BCME_ERROR;
	}

	WL_DBG(("WPA2 auth=0x%x wsec=0x%x wpa_auth=0x%x\n", auth[0], wsec[0], wpa_auth[0]));

	/* Parse WPA IE */
	if ((err = wl_validate_wpaie(dev, wpaie, bssidx)) < 0) {
		WL_ERR(("WPA IE error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_getint_bsscfg(dev, "auth", &auth[1], bssidx)) < 0) {
		WL_ERR(("WPA IE  auth error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_getint_bsscfg(dev, "wsec", &wsec[1], bssidx)) < 0) {
		WL_ERR(("WPA IE wsec error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_getint_bsscfg(dev, "wpa_auth", &wpa_auth[1], bssidx)) < 0) {
		WL_ERR(("WPA IE wpa_auth error %d\n", err));
		return BCME_ERROR;
	}

	WL_DBG(("WPA auth=0x%x wsec=0x%x wpa_auth=0x%x\n", auth[1], wsec[1], wpa_auth[1]));

	/* Set OR'ed values */
	auth[0] |= auth[1];
	wsec[0] |= wsec[1];
	wpa_auth[0] |= wpa_auth[1];

	if ((err = wldev_iovar_setint_bsscfg(dev, "auth", auth[0], bssidx)) < 0) {
		WL_ERR(("auth error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_setint_bsscfg(dev, "wsec", wsec[0], bssidx)) < 0) {
		WL_ERR(("wsec error %d\n", err));
		return BCME_ERROR;
	}

	if ((err = wldev_iovar_setint_bsscfg(dev, "wpa_auth", wpa_auth[0], bssidx)) < 0) {
		WL_ERR(("wpa_auth error %d\n", err));
		return BCME_ERROR;
	}

	WL_DBG(("WPA/WPA2 auth=0x%x wsec=0x%x wpa_auth=0x%x\n", auth[0], wsec[0], wpa_auth[0]));
...
#if 1	// 20120426 mixmode
				if (wpa_ie != NULL && wpa2_ie != NULL) {
					WL_DBG(("AP_CREATING: mixed mode\n"));
					if (wl_validate_wpamix_ie(dev, wpa2_ie, wpa_ie, bssidx)  < 0) {
...
#if 1  // 20120321 wps mark  workaround
				if ( (wl->ap_info) && (wl->ap_info->wps_ie) ) {
					WL_ERR(("wl->ap_info->wps_ie  CLEAR !! \n"));
					kfree(wl->ap_info->wps_ie);
					wl->ap_info->wps_ie = NULL;
				}
				if ( (wl->ap_info) && (wl->ap_info->beacon_ie_len > 0) ) {
					WL_ERR(("wl->ap_info->beacon_ie  CLEAR !! \n"));
					wl->ap_info->beacon_ie_len = 0;
					memset(wl->ap_info->beacon_ie,0, sizeof(wl->ap_info->beacon_ie));
				}
				if ( (wl->ap_info) && (wl->ap_info->probe_res_ie_len > 0) ) {
					WL_ERR(("wl->ap_info->probe_res_ie  CLEAR !! \n"));
					wl->ap_info->probe_res_ie_len = 0;
					memset(wl->ap_info->probe_res_ie,0, sizeof(wl->ap_info->probe_res_ie));
				}
#if 1  // 2012.03.29 FJ QA No.162-2
				if ( (wl->ap_info) && (wl->ap_info->assoc_res_ie_len > 0) ) {
					WL_ERR(("wl->ap_info->assoc_res_ie  CLEAR !! \n"));
					wl->ap_info->assoc_res_ie_len = 0;
					memset(wl->ap_info->assoc_res_ie,0, sizeof(wl->ap_info->assoc_res_ie));
				}
#endif // 2012.03.29 FJ QA No.162-2
#endif // 20120321 wps mark  workaround
...
#if 1 // 20120510 mixmode
				if (wpa_ie != NULL && wpa2_ie != NULL) {
					/* WPA/WPA2 mixed mode */
					if (!wl->ap_info->security_mode) {
						/* change from open mode to WPA/WPA2 mixed mode */
						WL_DBG(("No change in WPA IE for mixed mode\n"));
						update_bss = true;
						wl->ap_info->wpa_ie = kmemdup(wpa_ie,
						wpa_ie->length + WPA_RSN_IE_TAG_FIXED_LEN,
						GFP_KERNEL);
						...
						/* change from WPA, WPA2 or mixed mode */
						...
						if (memcmp(wl->ap_info->rsn_ie,
							...
							WL_DBG(("No change in RSN IE for mixed mode\n"));
					}
				} else
#endif // 20120510 mixmode
...
static s32 wl_event_handler(void *data)
{
	...
	DAEMONIZE("wl_event_handler"); // Fujitsu 2012.02.07 WLAN/BT add
