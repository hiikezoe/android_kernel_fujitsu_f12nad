...
#define IE_TYPE_LEN(type, bsstype) (wl_to_p2p_bss_saved_ie(wl, bsstype).p2p_ ## type ## _ie_len)
	if (p2p_is_on(wl) && bssidx != -1) {
		if (bssidx == P2PAPI_BSSCFG_PRIMARY)
			bssidx =  wl_to_p2p_bss_bssidx(wl, P2PAPI_BSSCFG_DEVICE);
//		CFGP2P_ERR(("bt_wlan: P2P case (pktflag=0x%x\n", pktflag));  // Fujitsu 2012.2.27 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
		switch (pktflag) {
...
//		CFGP2P_ERR(("bt_wlan: WL_MODE_AP case (pktflag=0x%x\n", pktflag));  // Fujitsu 2012.2.27 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
		switch (pktflag) {
			case VNDR_IE_PRBRSP_FLAG :
			...
			case VNDR_IE_ASSOCRSP_FLAG :
				CFGP2P_ERR((" VNDR_IE_ASSOCRSP_FLAG \n"));
...
//		CFGP2P_ERR(("bt_wlan: WL_MODE_BSS case (pktflag=0x%x\n", pktflag));  // Fujitsu 2012.2.27 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
		switch (pktflag) {
			case VNDR_IE_PRBREQ_FLAG :
...
//	CFGP2P_ERR(("bt_wlan: vndr_ie check, vndr_ie_len=%d \n", vndr_ie_len));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
	if (vndr_ie_len > mgmt_ie_buf_len) {
		CFGP2P_ERR(("extra IE size too big\n"));
		ret = -ENOMEM;
	} else {
//		CFGP2P_ERR(("bt_wlan: vndr_ie \"%s\" \n", vndr_ie));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
		if (mgmt_ie_buf != NULL) {
//			CFGP2P_ERR(("bt_wlan: mgmt IE is found \"%s\" \n", mgmt_ie_buf));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
			if (vndr_ie_len && (vndr_ie_len == *mgmt_ie_len) &&
			     (memcmp(mgmt_ie_buf, vndr_ie, vndr_ie_len) == 0)) {
				CFGP2P_INFO(("Previous mgmt IE is equals to current IE"));
//				CFGP2P_ERR(("bt_wlan: \n"));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
			    goto exit;
			}
//			CFGP2P_ERR(("bt_wlan: Previous mgmt IE is NOT equals to current IE \n"));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
			pos = 0;
			delete = 1;
			...
			while (pos < *mgmt_ie_len) {
				ie_id = ie_buf[pos++];
				ie_len = ie_buf[pos++];
//				CFGP2P_ERR(("bt_wlan:  ID : %d, Len : %d (DOT11_MNG_VS_ID=%d)\n", ie_id, ie_len, DOT11_MNG_VS_ID));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
//				CFGP2P_ERR(("bt_wlan:  wl_cfgp2p_is_wps_ie() ret=%d \n", wl_cfgp2p_is_wps_ie(&ie_buf[pos-2], NULL, 0)));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
...
//		CFGP2P_ERR(("bt_wlan: mgmt IE is NOT found (Initial registration)\"%s\" \n", mgmt_ie_buf));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
		*mgmt_ie_len = 0;
		/* Add if there is any extra IE */
		if (vndr_ie && vndr_ie_len) {
//			CFGP2P_ERR(("bt_wlan: save the current IE in wl struct, vndr_ie \"%s\", vndr_ie_len=%d \n", vndr_ie, vndr_ie_len));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
...
wl_cfgp2p_has_ie(u8 *ie, u8 **tlvs, u32 *tlvs_len, const u8 *oui, u32 oui_len, u8 type)
{
//	CFGP2P_ERR(("bt_wlan: Entry \n"));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
	/* If the contents match the OUI and the type */
	if (ie[TLV_LEN_OFF] >= oui_len + 1 &&
	        !bcmp(&ie[TLV_BODY_OFF], oui, oui_len) &&
	        type == ie[TLV_BODY_OFF + oui_len]) {
//	   CFGP2P_ERR(("bt_wlan: TRUE \n"));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
	   return TRUE;
	}

	if (tlvs == NULL)
		return FALSE;
//	CFGP2P_ERR(("bt_wlan: tlvs is NOT NULL (FALSE case2) \n"));  // Fujitsu 2012.3.5 WLAN/BT add, Fujitsu 2012.04.23 WLAN/BT del 
