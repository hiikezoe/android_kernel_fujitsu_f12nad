...
// Fujitsu 2012.03.13 wlan/bt broadcom Patch add start
	int rt;
	OSL_DELAY(1000);
// Fujitsu 2012.03.13 wlan/bt broadcom Patch add end
...
// Fujitsu 2012.03.13 wlan/bt broadcom Patch add start
    rt = bcmsdh_intr_disable(sdh);
    bus->intdis = TRUE;
    DHD_ERROR(("@@@@@@@@@ %s : retcode = %d \n", __FUNCTION__, rt));
// Fujitsu 2012.03.13 wlan/bt broadcom Patch add end
    	return;
	}

// Fujitsu 2013.03.13 broadcom Patch add start
//OSL_DELAY(1000);
// Fujitsu 2012.03.13 broadcom Patch add end
...
// Broadcom Patch Fujitsu 2011.10.04 del start
//        bcmsdh_intr_disable(bus->sdh);
//        bcmsdh_intr_dereg(bus->sdh);
// Broadcom Patch Fujitsu 2011.10.04 del end
...
// Broadcom Patch Fujitsu 2011.10.04 add start
        DHD_ERROR(("##### %s: De-register interrupt handler #####\n" ,__FUNCTION__));
        /* De-register interrupt handler */
        bcmsdh_intr_disable(bus->sdh);
        bcmsdh_intr_dereg(bus->sdh);
// Broadcom Patch Fujitsu 2011.10.04 add end
