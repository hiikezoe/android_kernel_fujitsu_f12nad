...
#define RJ6ABA_DRV_NAME  "rj6aba"

#define LOGI(fmt, args...)      printk(KERN_INFO "rj6aba: " fmt, ##args)
//#define LOGI(fmt, args...)      do{}while(0)
//#define LOGD(fmt, args...)      printk(KERN_DEBUG "rj6aba: " fmt, ##args)
#define LOGD(fmt, args...)      do{}while(0)
#define LOGE(fmt, args...)      printk(KERN_ERR "rj6aba: " fmt, ##args)
...
DEFINE_MUTEX(rj6aba_mtx);

struct rj6aba_work_t {
	struct work_struct work;
};
static DECLARE_WAIT_QUEUE_HEAD(rj6aba_wait_queue);
static struct rj6aba_work_t *rj6aba_sensorw;
static struct rj6aba_ctrl *rj6aba_ctrl = NULL;
static struct i2c_client *rj6aba_i2c_client = NULL;
...
// I2C
////////////////////////////////
int rj6aba_i2c_write(struct CameraSensorI2CCmdTypeIsp* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_front_write(rj6aba_i2c_client, pI2CCmd);
	} else {
...
int rj6aba_i2c_read(struct CameraSensorI2CCmdTypeIsp* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_front_read(rj6aba_i2c_client, pI2CCmd);
	} else {
...
int rj6aba_i2c_write_normal(struct CameraSensorI2CCmdType* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_front_write_normal(rj6aba_i2c_client, pI2CCmd);
	} else {
...
int rj6aba_i2c_read_normal(struct CameraSensorI2CCmdType* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_front_read_normal(rj6aba_i2c_client, pI2CCmd);
	} else {
...
static int rj6aba_sensor_poweron(void)
{
    int i;
...
// Power OFF
////////////////////////////////
static void rj6aba_sensor_poweroff(void)
{
    int i;
...
#if 1
		gpio_direction_output(CAM_I2C_SDA, 0);
		gpio_direction_output(CAM_I2C_SCL, 0);
#else
...
// msm_open_control
//---------------------------------------------------------------------
int rj6aba_sensor_init(const struct msm_camera_sensor_info *data)
{
    LOGI("+%s()\n", __func__);
    
    rj6aba_ctrl = kzalloc(sizeof(struct rj6aba_ctrl), GFP_KERNEL);
    if (!rj6aba_ctrl) {
    ...
    if (data)
        rj6aba_ctrl->sensordata = data;

    // Sensor Power ON
    if (rj6aba_sensor_poweron() < 0) {
        kfree(rj6aba_ctrl);
        LOGE("-%s Failed.\n", __func__);
        return -1;
    }
...
static int32_t rj6aba_video_config(int mode)
{
    int32_t rc = 0;
#if 0 // for MIPI 
    struct msm_camera_csi_params rj6aba_csi_params;
    LOGI("-%s top.\n", __func__);
    msm_camio_vfe_clk_rate_set(192000000);
    rj6aba_csi_params.data_format = CSI_10BIT;
    rj6aba_csi_params.lane_cnt = 2;
    rj6aba_csi_params.lane_assign = 0xe4;
    rj6aba_csi_params.dpcm_scheme = 0;
    rj6aba_csi_params.settle_cnt = 0x18;
    rc = msm_camio_csi_config(&rj6aba_csi_params);
    if (rc < 0)
...
static int32_t rj6aba_set_sensor_mode(int mode,
    int res)
{
    int32_t rc = 0;
    LOGI("-%s top.\n", __func__);
...
// msm_ioctl_control()
//---------------------------------------------------------------------
int rj6aba_sensor_config(void __user *argp)
{
    ...
    case CFG_PWR_UP:
        LOGI("-%s (CFG_PWR_UP)\n", __func__);
        // Sensor Power OFF
        rj6aba_sensor_poweroff();

        mdelay(10);

        // Sensor Power ON
        rc =rj6aba_sensor_poweron();
        if(rc < 0) {
    ...
    case CFG_COMMAND_NORMAL:
		...
		if (!cfg.cfg.cmd.rxlen) {
        	rc = rj6aba_i2c_write_normal(&I2CCmdNormal);
		} else {
        	rc = rj6aba_i2c_read_normal(&I2CCmdNormal);
			if( !rc ) {
    ...
    case CFG_SET_MODE:
        LOGD("-%s (CFG_SET_MODE)\n", __func__);
        rc = rj6aba_set_sensor_mode(cfg.mode, cfg.rs);
        break;
...
// msm_release_control()
//---------------------------------------------------------------------
int rj6aba_sensor_release(void)
{
    LOGI("+%s\n", __func__);
...
static const struct i2c_device_id rj6aba_i2c_id[] = {
	{ RJ6ABA_DRV_NAME, 0},
	{ },
};

static int rj6aba_init_client(struct i2c_client *client)
{
    LOGI("+%s()\n", __func__);
	/* Initialize the MSM_CAMI2C Chip */
	init_waitqueue_head(&rj6aba_wait_queue);
	return 0;
}


static int __devinit rj6aba_i2c_probe( struct i2c_client *client, const struct i2c_device_id *id)
{
	int rc = 0;
	...
	rj6aba_sensorw = kzalloc(sizeof(struct rj6aba_work_t), GFP_KERNEL);
	if (!rj6aba_sensorw) {
		LOGE("kzalloc failed.\n");
		rc = -ENOMEM;
		return rc;
	...
	i2c_set_clientdata(client, rj6aba_sensorw);
	rj6aba_init_client(client);
	rj6aba_i2c_client = client;

	msleep(50);

	LOGI("rj6aba_probe successed! rc = %d\n", rc);
	return 0;

}

static struct i2c_driver rj6aba_i2c_driver = {
	.id_table = rj6aba_i2c_id,
	.probe  = rj6aba_i2c_probe,
	.remove = __exit_p(rj6aba_i2c_remove),
	.driver = {
		.name = RJ6ABA_DRV_NAME,
	},
};

static int rj6aba_sensor_probe(const struct msm_camera_sensor_info *info,
                                struct msm_sensor_ctrl *s)
{
	...
	if( is_use_qup_i2c ) {
		rc = i2c_add_driver( &rj6aba_i2c_driver );
		if( rc < 0 ) {
			rc = -ENOTSUPP;
			return rc;
		}
    ...
    s->s_init = rj6aba_sensor_init;
    s->s_release = rj6aba_sensor_release;
    s->s_config  = rj6aba_sensor_config;
...
static int __rj6aba_probe(struct platform_device *pdev)
{
...
/* FUJITSU : 2012-01-19 add camera end */
    ...
    return msm_camera_drv_start(pdev, rj6aba_sensor_probe);
...
static int __init rj6aba_init(void)
{
    LOGI("+%s()\n", __func__);
    return platform_driver_register(&msm_camera_driver);
}

module_init(rj6aba_init);
