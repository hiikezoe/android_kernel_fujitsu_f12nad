...
#define M6MORL_DRV_NAME  "m6morl_imx091"

#define LOGI(fmt, args...)      printk(KERN_INFO "m6morl_imx091: " fmt, ##args)
//#define LOGI(fmt, args...)      do{}while(0)
//#define LOGD(fmt, args...)      printk(KERN_DEBUG "m6morl_imx091: " fmt, ##args)
#define LOGD(fmt, args...)      do{}while(0)
#define LOGE(fmt, args...)      printk(KERN_ERR "m6morl_imx091: " fmt, ##args)
...
/*===================================================================*
    LOCAL DECLARATIONS
 *===================================================================*/
struct m6morl_imx091_ctrl {
    const struct msm_camera_sensor_info *sensordata;
//    int model;
//    int led;
};

DEFINE_MUTEX(m6morl_imx091_mtx);

struct m6morl_imx091_work_t {
	struct work_struct work;
};
static DECLARE_WAIT_QUEUE_HEAD(m6morl_imx091_wait_queue);
static struct m6morl_imx091_work_t *m6morl_imx091_sensorw;
static struct m6morl_imx091_ctrl *m6morl_imx091_ctrl = NULL;
static struct i2c_client *m6morl_imx091_i2c_client = NULL;
...
// I2C
////////////////////////////////
int m6morl_imx091_i2c_write(struct CameraSensorI2CCmdTypeIsp* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_write(m6morl_imx091_i2c_client, pI2CCmd);
	} else {
...
int m6morl_imx091_i2c_read(struct CameraSensorI2CCmdTypeIsp* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_read(m6morl_imx091_i2c_client, pI2CCmd);
	} else {
...
int m6morl_imx091_i2c_write_normal(struct CameraSensorI2CCmdType* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_write_normal(m6morl_imx091_i2c_client, pI2CCmd);
	} else {
...
int m6morl_imx091_i2c_read_normal(struct CameraSensorI2CCmdType* pI2CCmd)
{
	int rc = 0;
	if( is_use_qup_i2c ) {
		rc = camsensor_i2c_read_normal(m6morl_imx091_i2c_client, pI2CCmd);
	} else {
...
static int m6morl_imx091_sensor_poweron(void)
{
    int i;
...
// Power OFF
////////////////////////////////
static void m6morl_imx091_sensor_poweroff(void)
{
    int i;
...
// msm_open_control
//---------------------------------------------------------------------
int m6morl_imx091_sensor_init(const struct msm_camera_sensor_info *data)
{
	
    LOGI("+%s()\n", __func__);
    
    m6morl_imx091_ctrl = kzalloc(sizeof(struct m6morl_imx091_ctrl), GFP_KERNEL);
    if (!m6morl_imx091_ctrl) {
        LOGE(" -%s kzalloc() Failed!\n",__func__);
        return -ENOMEM;
    }

    if (data)
        m6morl_imx091_ctrl->sensordata = data;

    // Sensor Power ON
    if (m6morl_imx091_sensor_poweron() < 0) {
        kfree(m6morl_imx091_ctrl);
        LOGE("-%s Failed.\n", __func__);
        return -1;
    }
...
static int32_t m6morl_imx091_video_config(int mode)
{
    struct msm_camera_csi_params m6morl_imx091_csi_params;
    int32_t rc = 0;
    LOGI("-%s top.\n", __func__);
    msm_camio_vfe_clk_rate_set(192000000);
    m6morl_imx091_csi_params.data_format = CSI_10BIT;
    m6morl_imx091_csi_params.lane_cnt = 2;
    m6morl_imx091_csi_params.lane_assign = 0xe4;
    m6morl_imx091_csi_params.dpcm_scheme = 0;
    m6morl_imx091_csi_params.settle_cnt = 0x18;
    rc = msm_camio_csi_config(&m6morl_imx091_csi_params);
    if (rc < 0)
...
static int32_t m6morl_imx091_set_sensor_mode(int mode,
    int res)
{
    int32_t rc = 0;
    LOGD("-%s top.\n", __func__);
    ...
    case SENSOR_PREVIEW_MODE:
        ...
        rc = m6morl_imx091_video_config(mode);
        break;
...
// msm_ioctl_control()
//---------------------------------------------------------------------
int m6morl_imx091_sensor_config(void __user *argp)
{
    ...
    case CFG_PWR_UP:
        LOGI("-%s (CFG_PWR_UP)\n", __func__);
        // Sensor Power OFF
        m6morl_imx091_sensor_poweroff();

        mdelay(10);

        // Sensor Power ON
        rc =m6morl_imx091_sensor_poweron();
        if(rc < 0) {
...
//					LOGD("+i2c_read (%02x)\n", cfg.cfg.cmd_isp.rvalue[cpl] );
    ...
    case CFG_COMMAND_NORMAL:
		...
		if (!cfg.cfg.cmd.rxlen) {
        	rc = m6morl_imx091_i2c_write_normal(&I2CCmdNormal);
		} else {
        	rc = m6morl_imx091_i2c_read_normal(&I2CCmdNormal);
			if( !rc ) {
    ...
    case CFG_SET_MODE:
        LOGD("-%s (CFG_SET_MODE)\n", __func__);
        rc = m6morl_imx091_set_sensor_mode(cfg.mode, cfg.rs);
        break;
    ...
    mutex_unlock(&m6morl_imx091_mtx);
...
// msm_release_control()
//---------------------------------------------------------------------
int m6morl_imx091_sensor_release(void)
{
    LOGI("+%s\n", __func__);
...
static const struct i2c_device_id m6morl_imx091_i2c_id[] = {
	{ M6MORL_DRV_NAME, 0},
	{ },
};

static int m6morl_imx091_init_client(struct i2c_client *client)
{
    LOGI("+%s()\n", __func__);
	/* Initialize the MSM_CAMI2C Chip */
	init_waitqueue_head(&m6morl_imx091_wait_queue);
	return 0;
}


static int __devinit m6morl_imx091_i2c_probe( struct i2c_client *client, const struct i2c_device_id *id)
{
	int rc = 0;
	...
	m6morl_imx091_sensorw = kzalloc(sizeof(struct m6morl_imx091_work_t), GFP_KERNEL);
	if (!m6morl_imx091_sensorw) {
		LOGE("kzalloc failed.\n");
		rc = -ENOMEM;
		return rc;
	}

	i2c_set_clientdata(client, m6morl_imx091_sensorw);
	m6morl_imx091_init_client(client);
	m6morl_imx091_i2c_client = client;

	msleep(50);

	LOGI("m6morl_imx091_probe successed! rc = %d\n", rc);
	return 0;

}

static struct i2c_driver m6morl_imx091_i2c_driver = {
	.id_table = m6morl_imx091_i2c_id,
	.probe  = m6morl_imx091_i2c_probe,
	.remove = __exit_p(m6morl_imx091_i2c_remove),
	.driver = {
		.name = M6MORL_DRV_NAME,
	},
};

static int m6morl_imx091_sensor_probe(const struct msm_camera_sensor_info *info,
                                struct msm_sensor_ctrl *s)
{
	...
	if( is_use_qup_i2c ) {
		rc = i2c_add_driver( &m6morl_imx091_i2c_driver );
		if( rc < 0 ) {
			rc = -ENOTSUPP;
			return rc;
		}
    ...
    s->s_init = m6morl_imx091_sensor_init;
    s->s_release = m6morl_imx091_sensor_release;
    s->s_config  = m6morl_imx091_sensor_config;
...
static int __m6morl_imx091_probe(struct platform_device *pdev)
{
...
/* FUJITSU : 2012-01-19 add camera end */
    ...
    return msm_camera_drv_start(pdev, m6morl_imx091_sensor_probe);
...
static int __init m6morl_imx091_init(void)
{
    LOGI("+%s()\n", __func__);
    return platform_driver_register(&msm_camera_driver);
}

module_init(m6morl_imx091_init);
