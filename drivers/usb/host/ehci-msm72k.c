...
/* FUJITSU:2012-06-27 USB start */
static unsigned int enable_otg = 1;
static int store_enable_host(const char *val, struct kernel_param *kp);

module_param_call(enable_otg, store_enable_host, NULL, &enable_otg
							, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
MODULE_PARM_DESC(enable_otg, "EHCI enable host control");
...
static int store_enable_host(const char *val, struct kernel_param *kp)
{
	...
	pr_info("%s: enable_host = %d", __func__, enable_otg);

	return otg_set_vbus(xceiv, !!enable_otg);
}
/* FUJITSU:2012-06-27 USB end */
