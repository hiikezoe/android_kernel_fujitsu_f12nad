...
struct mtp_dev {
	...
	int xfer_result;
/* FUJITSU:2012-03-02 USB start */
	atomic_t in_xfer;
	atomic_t xfer_force_stop;
/* FUJITSU:2012-03-02 USB end */
...
/* FUJITSU:2012-03-02 USB start */
		if (atomic_read(&dev->xfer_force_stop)) {
			pr_info("%s %s : force stop !", mtp_shortname, __func__);
			dev->state = STATE_OFFLINE;
			r = -ENODEV;
			break;
		}
...
/* FUJITSU:2012-03-02 USB start */
	atomic_set(&dev->xfer_force_stop, 0);
/* FUJITSU:2012-03-02 USB end */
...
mtp_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	int i;
/* FUJITSU:2012-03-02 USB start */
	if (atomic_read(&dev->in_xfer)) {
		atomic_set(&dev->xfer_force_stop, 1);
		while (atomic_read(&dev->in_xfer))
			msleep(10);
	}
/* FUJITSU:2012-03-02 USB end */
