...
/* FUJITSU:2012-03-08 USB start */
	dev_info(&ui->pdev->dev, "reset controller done\n");
/* FUJITSU:2012-03-08 USB end */
}
...
/* FUJITSU:2012-03-08 USB start */
static int msm72k_flush_endpoint(struct usb_gadget *_gadget)
{
	struct usb_info * ui = container_of(_gadget, struct usb_info, gadget);

	if (!ui) {
		pr_err("msm72k_udc: missing usb_info.\n");
		return -EINVAL;
	}
...
/* FUJITSU:2012-03-08 USB start */
	.flush_endpoint	= msm72k_flush_endpoint,
/* FUJITSU:2012-03-08 USB end */
};
...
/* FUJITSU:2012-02-23 USB start */
	if (otg->pdata->atom_notif_init)
		otg->pdata->atom_notif_init(CABLE_DET_USB_DRV);
