...
/* FUJITSU:2011-12-15 USB start */
#include <linux/usb_id.h>
/* FUJITSU:2011-12-15 USB end */

#include "gadget_chips.h"

/* FUJITSU:2012-01-06 USB start */
#define CONFIG_USB_ANDROID_OM y
#define CONFIG_USB_ANDROID_MMP y
/* FUJITSU:2012-01-06 USB end */
/*
...
/* FUJITSU:2012-05-15 USB start */
#define	CONFIG_USB_ANDROID_RNDIS	// Generic default
#if defined(CONFIG_MACH_F12NAD)
#undef	CONFIG_USB_ANDROID_RNDIS
#endif
#ifdef CONFIG_USB_ANDROID_RNDIS
/* FUJITSU:2012-05-15 USB end */
...
/* FUJITSU:2012-02-27 USB start */
#ifdef ENABLE_NVREAD_IF
extern void debugusb_set_pid_to_nv(unsigned long pid);
#endif /* ENABLE_NVREAD_IF */
/* FUJITSU:2012-02-27 USB end */
/* FUJITSU:2012-05-15 USB start */
extern int debugusb_allow_portchange(unsigned long pid);
/* FUJITSU:2012-05-15 USB end */
...
static int android_get_function_enable(char *name)
{
...
/* FUJITSU:2011-12-26 USB start */
//		goto bind_config;
		goto serial_setup;
/* FUJITSU:2011-12-26 USB end */
...
{
/* FUJITSU:2012-03-07 USB start */
	if (android_get_function_enable("adb"))
...
#ifdef CONFIG_USB_ANDROID_OM
/* OM */
static int om_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	return om_setup();
}
static void om_function_cleanup(struct android_usb_function *f)
{
	om_cleanup();
}
static int om_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return om_bind_config(c, 0);
}

static struct android_usb_function om_function = {
	.name		= "om",
	.init		= om_function_init,
	.cleanup	= om_function_cleanup,
	.bind_config	= om_function_bind_config,
};
static int dm_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return dm_bind_config(c, 0);
}
...
static int da_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	return da_bind_config(c, 0);
}

static struct android_usb_function da_function = {
	.name		= "da",
	.bind_config	= da_function_bind_config,
	.port_count = 1,
};
#endif
/* FUJITSU:2012-01-06 USB end */
...
/* FUJITSU:2012-01-06 USB start */
		gserial_port += f->port_count;
/* FUJITSU:2012-01-06 USB end */
	}
/* FUJITSU:2012-01-06 USB start */
	pr_info("%s: gserial_port[%d]\n", __func__, gserial_port);
	if (gserial_port == 2) {
		gserial_setup(dev->cdev->gadget, gserial_port + 1);
		pr_info("%s: gserial_setup [%d]\n", __func__,gserial_port);
	}
/* FUJITSU:2012-01-06 USB end */
	return 0;
}
...
{
	struct android_usb_function *f;
/* FUJITSU:2012-01-12 USB start */
	int gserial_port = 0;
/* FUJITSU:2012-01-12 USB end */
...
/* FUJITSU:2012-05-15 USB start */
		if (!debugusb_allow_portchange((unsigned long)device_desc.idProduct)) {
			pr_info("%s: could not change new port: %x\n", __func__, device_desc.idProduct);
			return size;
		}
/* FUJITSU:2012-05-15 USB end */
		
/* FUJITSU:2012-01-06 USB start */
		if (device_desc.idProduct == USB_ID_ALADN) {
...
/* FUJITSU:2012-02-27 USB start */
#ifdef ENABLE_NVREAD_IF
		debugusb_set_pid_to_nv((unsigned long)device_desc.idProduct);
#endif /* ENABLE_NVREAD_IF */
/* FUJITSU:2012-02-27 USB end */
	} else if (!enabled && dev->enabled) {
		usb_gadget_disconnect(cdev->gadget);
/* FUJITSU:2012-03-08 USB start */
		usb_gadget_flush_endpoint(cdev->gadget);
/* FUJITSU:2012-03-08 USB end */
...
/* FUJITSU:2012-01-04 USB start */
#if 0
	ret = android_init_functions(dev->functions, cdev);
	if (ret)
		return ret;
#endif
/* FUJITSU:2012-01-04 USB end */
...
/* FUJITSU:2012-04-23 USB start */
#if defined(CONFIG_MACH_F12APON)
	strlcpy(manufacturer_string, "Fujitsu", sizeof(manufacturer_string) - 1);
	strlcpy(product_string, "DOCOMO F07D", sizeof(product_string) - 1);
	strlcpy(serial_string, "F07D000000", sizeof(serial_string) - 1);
#elif defined(CONFIG_MACH_F09D)
	strlcpy(manufacturer_string, "Fujitsu", sizeof(manufacturer_string) - 1);
	strlcpy(product_string, "DOCOMO F09D", sizeof(product_string) - 1);
	strlcpy(serial_string, "F09D000000", sizeof(serial_string) - 1);
#elif defined(CONFIG_MACH_F12NAD)
	strlcpy(manufacturer_string, "Fujitsu", sizeof(manufacturer_string) - 1);
	strlcpy(product_string, "DOCOMO F12D", sizeof(product_string) - 1);
	strlcpy(serial_string, "F12D000000", sizeof(serial_string) - 1);
#else
