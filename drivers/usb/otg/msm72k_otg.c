...
/* FUJITSU:2012-05-11 USB (qcom patch for CR334991) start */
		if (on) {
			otg_pm_qos_update_latency(dev, 1);
		}else {
			otg_pm_qos_update_latency(dev, 0);
		}
/* FUJITSU:2012-05-11 USB (qcom patch for CR334991) end */
...
/* FUJITSU:2012-02-23 USB start */
	if ((dev->otg.state == OTG_STATE_B_IDLE)
	 || (dev->otg.state == OTG_STATE_A_IDLE)) {
	 	if (dev->pdata->phy_power
	 	 && dev->pdata->atom_notif_cable_disconnect) {
	 		if (!dev->first)
	 			dev->first = 1;
	 		else {
	 			dev->pdata->phy_power(USB_PHY_PWR_OFF);
	 			dev->pdata->atom_notif_cable_disconnect();
	 		}
	 	}
	}
...
/* FUJITSU:2012-02-23 USB start */
	if ((dev->otg.state == OTG_STATE_B_IDLE)
	 || (dev->otg.state == OTG_STATE_A_IDLE)) {
	 	if (dev->pdata->phy_power) {
	 		if (dev->pdata->phy_power(USB_PHY_PWR_ON))
	 			goto out;
	 	}
	}
/* FUJITSU:2012-02-23 USB end */
...
/* FUJITSU:2012-06-26 USB start */
#ifdef CONFIG_USB_MSM_72K
static int msm_otg_set_vbus(struct otg_transceiver *xceiv, bool enabled)
{
	struct msm_otg *dev = the_msm_otg;
	int ret = 0;

	if (!xceiv) {
		pr_err("%s : xceiv is null", __func__);
		ret = -ENODEV;
		goto leave;
	}

	pr_info("%s : enabled=%d, host connect=%d, state = %s\n" , __func__
					, enabled, is_host(), state_string(xceiv->state));

	/* vbus on/off only if connecting host device */
	if (is_host()
		&& (dev->otg.state != OTG_STATE_A_IDLE)
		&& (dev->otg.state != OTG_STATE_B_IDLE)) {
		if (dev->pdata->vbus_power)
			dev->pdata->vbus_power(USB_PHY_INTEGRATED, enabled);
	}
leave:
	return ret;
}
#endif
/* FUJITSU:2012-06-26 USB end */
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(ID, &dev->inputs) ||
			...
			otg_pm_qos_update_latency(dev, 0);
/* FUJITSU:2012-02-23 USB start */
			if (dev->pdata->atom_notif_usb_vdd_off)
				dev->pdata->atom_notif_usb_vdd_off();
/* FUJITSU:2012-02-23 USB end */
		}
		break;
	case OTG_STATE_B_WAIT_ACON:
...
/* FUJITSU:2012-06-26 USB start */
	dev->otg.set_vbus = msm_otg_set_vbus;
/* FUJITSU:2012-06-26 USB end */
#endif
