 ...
 * ======================== ========== ===============  =======================================
 * 121-8x55-PARTS1-0037     2012/01/18 FST          
===============================================================================================
 */
...
/* FUJITSU:2011-10-24 FTA add start */
...
//	now = current_kernel_time(); /* FUJITSU:2011-10-24 FTA del */
	/* FUJITSU:2011-10-24 FTA add start */
	{
		unsigned long flags;
		preempt_disable();
...
{
	/* FUJITSU:2012-01-18_[P]-11-2ND-GEN_121-8x55-PARTS1-0037_BUGFIX-GEN_ADD-S */
	struct logger_log *log;
	/* FUJITSU:2012-01-18_[P]-11-2ND-GEN_121-8x55-PARTS1-0037_BUGFIX-GEN_ADD-E */
	if (file->f_mode & FMODE_READ) {
		struct logger_reader *reader = file->private_data;
		/* FUJITSU:2012-01-18_[P]-11-2ND-GEN_121-8x55-PARTS1-0037_BUGFIX-GEN_ADD-S */
		log = reader->log;
 ...
 */
/* FUJITSU:2011-10-24 FTA del start */
#if 0
...
/* FUJITSU:2011-12-01 add log for event/radio end */
...
#endif
/* FUJITSU:2011-10-24 FTA del end */
...
/* FUJITSU:2011-10-24 FTA add end */
...
static int __init logger_init(void)
{
	...
	ret = init_log(FTA_LOG_MAIN, LOGGER_LOG_MAIN); /* FUJITSU:2011-10-24 FTA mod */
	if (unlikely(ret))
		goto out;

	ret = init_log(FTA_LOG_EVENTS, LOGGER_LOG_EVENTS); /* FUJITSU:2011-10-24 FTA mod */
	if (unlikely(ret))
		goto out;

	ret = init_log(FTA_LOG_RADIO, LOGGER_LOG_RADIO); /* FUJITSU:2011-10-24 FTA mod */
	if (unlikely(ret))
		goto out;

	ret = init_log(FTA_LOG_SYSTEM, LOGGER_LOG_SYSTEM); /* FUJITSU:2011-10-24 FTA mod */
	if (unlikely(ret))
		goto out;
