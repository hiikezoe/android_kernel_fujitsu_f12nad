...
static int msmsdcc_enable(struct mmc_host *mmc)
{
	...
	struct msmsdcc_host *host = mmc_priv(mmc);
//	unsigned long flags; 	// Fujitsu 2012.04.02 WLAN/BT add -> Fujitsu 2012.05.31 WLAN/BT del

	msmsdcc_pm_qos_update_latency(host, 1);

#if 0 	// Fujitsu 2012.05.31 WLAN/BT del start
#if 1 	// Fujitsu 2012.04.02 WLAN/BT add start
	spin_lock_irqsave(&host->lock, flags);
	if (!host->clks_on) {
		msmsdcc_setup_clocks(host, true);
		host->clks_on = 1;
	...
	spin_unlock_irqrestore(&host->lock, flags);
#endif	// Fujitsu 2012.04.02 WLAN/BT add end
#endif	// Fujitsu 2012.05.31 WLAN/BT del end
...
// Fujitsu 2012.01.06 WLAN/BT change start
		...
		mmc_detect_change(host->mmc, msecs_to_jiffies(6000));
...
// Fujitsu 2012.1.7 WLAN/BT add start


static void


set_polling_tts(struct msmsdcc_host *host, bool poll_on)
...
// Fujitsu 2012.1.7 WLAN/BT add end
...
// Fujitsu 2012.1.7 WLAN/BT add start
	...
	for (i = 0; i < pdev->num_resources; i++) {
		...
		if (!strncmp(pdev->resource[i].name,
			...
			// SDIO polling ON, for msm_sdcc.3 (WLAN)


			set_polling_tts(host, 1);


			printk(KERN_ERR "%s: SDIO polling ON(%s)\n", __func__, pdev->resource[i].name);
...
// Fujitsu 2012.02.03 WLAN/BT change start
...
// Fujitsu 2012.02.03 WLAN/BT change end
...
#if 1	// Fujitsu 2012.04.02 WLAN/BT change Start
			/*
...
#endif	// Fujitsu 2012.04.02 WLAN/BT change end
		}
