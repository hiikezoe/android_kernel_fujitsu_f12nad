/*
 * Copyright(C) 2009-2012 FUJITSU LIMITED
 *
...
static int crashdump_erasepartition_mtd(void)
{
	...
	dprintk(KERN_EMERG "(%s:%s:%d erase start %08x)\n", __FILE__, __FUNCTION__, __LINE__, (unsigned int)instr.addr);
	while (1) {
		if ( instr.addr >= crashdump_size ) {
			break;
		}
		...
		instr.addr += instr.mtd->erasesize;
	}
	dprintk(KERN_EMERG "(%s:%s:%d erase end %08x)\n", __FILE__, __FUNCTION__, __LINE__, (unsigned int)instr.addr);
...
static int crashdump_writedo_mtd(char *abuf, int alen)
{
	...
	if ( crashdump_part->mtd.panic_write ) {
		retval = crashdump_part->mtd.panic_write(&crashdump_part->mtd, off, len, &retlen, buf);
	} else {
		retval = crashdump_part->mtd.write(&crashdump_part->mtd, off, len, &retlen, buf);
	}
	if (retval < 0) {
